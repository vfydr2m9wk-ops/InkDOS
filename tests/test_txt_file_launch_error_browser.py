...
visible error in Plain Text instead of failing silently (inkdos:file-launch-error)."""
...
VISIBLE=r"""()=>{const s=[...document.querySelectorAll('.state-error')].find(e=>e.getBoundingClientRect().height>0);return s?s.innerText:null}"""
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            for path in ('picker','launchQueue'):
                ...
                page.wait_for_function("() => document.body.dataset.runtimeReady === 'true'")
                if path=='picker':
                ...
                assert 'open failed' in text.lower(),(path,text)
                ctx.close()
            browser.close()
    finally:
        server.terminate();server.wait(timeout=5)
    print(f'Plain Text file-launch error visibility ({browser_name}): OK')
