...
def test_two_action_apps_use_identical_labels_order_and_icons():
    for module,(new_id,open_id) in TWO.items():
        ...
        open_el=(doc.select_one('.inkdesk-start-open') if module == 'documents' else doc.find(id=open_id))
        assert new and open_el
        assert text(new) == "+ New document"
        assert text(open_el) == "Open document"
        assert "inkdesk-start-new" in new.get("class", [])
        assert "inkdesk-start-open" in open_el.get("class", [])
        assert new.select_one('.inkdesk-plus') is not None
        assert open_el.select_one('.inkdesk-folder svg') is not None
...
def test_one_action_apps_expose_only_uniform_open_action():
    for module,open_id in ONE.items():
        ...
        assert "inkdesk-start-open" in open_el.get("class", [])
        assert open_el.select_one('.inkdesk-folder svg') is not None
        parent=open_el.find_parent(class_='inkdesk-start-actions')
        assert parent and "inkdesk-start-actions-single" in parent.get("class", [])
        assert parent.select_one('.inkdesk-start-new') is None
...
def test_shared_css_preserves_current_app_colors_and_button_contrast():
    ...
    assert "background:var(--inkdesk-start-accent)!important" in css
