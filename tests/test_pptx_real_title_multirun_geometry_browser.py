...
def main() -> None:
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            synthetic = page.evaluate(
                """async runs => {
                    ...
                    title.paragraphs=[{
                        ...
                        runs:[{text:full,fontSizePt:112.5,bold:true,italic:false,underline:false,color:'#F3EEE4',fontFamily:'Arial'}]
                    }];
                    ...
                    const target=Array.from(doc.getElementsByTagNameNS(a,'r')).find(r=>
                        Array.from(r.getElementsByTagNameNS(a,'t')).some(t=>t.textContent===full)
                    );
                    ...
                    for(const text of runs){
                        ...
                        parent.insertBefore(run,target);
                    }
                    const txBody=parent.parentNode;
                    ...
                    bodyPr.setAttribute('tIns','25400');bodyPr.setAttribute('bIns','25400');
                    bodyPr.setAttribute('wrap','square');
                    for(const child of Array.from(bodyPr.children)){
                        if(child.localName==='spAutoFit'||child.localName==='noAutofit')child.remove();
                    }
                    if(!Array.from(bodyPr.children).some(n=>n.localName==='normAutofit')){
            ...
            page.evaluate(
                """async bytes => {
                    const file=new File([new Uint8Array(bytes)],'real-title-multirun-geometry.pptx',{
            ...
            geometry = page.evaluate(
                """runs => {
                    ...
                    return {
                        ...
                        renderedRuns:spans.map(s=>s.textContent)
                    };
                }""",
                RUNS,
            )
