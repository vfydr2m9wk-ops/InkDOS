...
visible error in EPUB instead of failing silently (inkdos:file-launch-error)."""
...
VISIBLE=r"""()=>{const s=[...document.querySelectorAll('footer,.statusbar,.statusbar *')].find(e=>e.getBoundingClientRect().height>0&&/Open failed|invalid-zip/.test(e.innerText));return s?s.innerText:null}"""
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            for path in ('picker','launchQueue'):
                ...
                assert ('open failed' in text.lower()) if path=='picker' else ('invalid-zip' in text.lower()),(path,text)
                ctx.close()
            browser.close()
    finally:
        server.terminate();server.wait(timeout=5)
    print(f'EPUB file-launch error visibility ({browser_name}): OK')
