"""Behavioral regression for InkDOS v0.20.3.1 functional corrections."""
...
def test_spreadsheet_rename(page, base_url, checks):
    ...
    recovered = page.evaluate("async () => await window.__InkDOSSpreadsheetsRecovery.capture()")
...
def test_spreadsheet_history_safety(page, base_url, checks):
    ...
    formula.fill("three")
    formula.press("Enter")
    page.evaluate("async payload => await window.__InkDOSSpreadsheetsRecovery.restore({snapshot:{payload},source:null})", recovered)
...
def test_presentation_rename(page, base_url, checks):
    ...
    recovered = page.evaluate("async () => await window.__InkDOSPresentationsRecovery.capture()")
