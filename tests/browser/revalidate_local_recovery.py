...
def reopen_and_restore(context, url):
    ...
    page.wait_for_selector(".inkdos-recovery-overlay", state="visible", timeout=10000)
    page.get_by_role("button", name="Restore", exact=True).click()
    page.wait_for_selector(".inkdos-recovery-overlay", state="detached", timeout=10000)
    return page, errors
...
def documents_case(browser, base_url):
    ...
    page.evaluate("window.__InkDOSDocumentsRecovery.manager.flush()")
    snapshots = page.evaluate("InkDOSLocalRecovery.listSnapshots('documents').then(items=>items.length)")
    if snapshots < 1:
    ...
    restored.evaluate("InkDOSLocalRecovery.clearModule('documents')")
    restored.close()
    context.close()
...
def spreadsheets_case(browser, base_url):
    ...
    page.wait_for_function("window.InkDOSSpreadsheetFormulaEditor?.hasPendingDrafts() === true")
    page.evaluate("window.__InkDOSSpreadsheetsRecovery.manager.flush()")
    snapshots = page.evaluate("InkDOSLocalRecovery.listSnapshots('spreadsheets').then(items=>items.length)")
    if snapshots < 1:
    ...
    capture = restored.evaluate("window.__InkDOSSpreadsheetsRecovery.capture()")
    value = next((item[1].get("v") for item in capture["book"]["sheets"][0]["cells"] if item[0] == "A1"), None)
    restored_drafts = restored.evaluate("window.InkDOSSpreadsheetFormulaEditor.snapshotDrafts()")
    draft_visible = restored.locator('.cell[data-r="1"][data-c="1"]').inner_text() == formula_draft
    restored.click('.cell[data-r="1"][data-c="1"]')
    restored.wait_for_function("window.InkDOSSpreadsheetFormulaEditor?.isActive() === true")
    resumed_value = restored.evaluate("window.InkDOSSpreadsheetFormulaEditor.getValue()")
    restored.evaluate("InkDOSLocalRecovery.clearModule('spreadsheets')")
    restored.close()
    context.close()
...
def spreadsheets_post_save_edit_recovery_case(browser, base_url):
    ...
    page.goto(url, wait_until="networkidle")
    page.evaluate("InkDOSLocalRecovery.clearModule('spreadsheets')")
    ...
    page.wait_for_function("InkDOSLocalRecovery.listSnapshots('spreadsheets').then(items=>items.length>=1)")
    protected_after_download = page.evaluate("""async () => ({
      snapshots: (await InkDOSLocalRecovery.listSnapshots('spreadsheets')).length,
...
def recovery_write_barrier_case(browser, base_url):
    ...
    result = page.evaluate("""async () => {
      await InkDOSLocalRecovery.clearModule('recovery-race');
      let releaseSerialize;
      const manager = InkDOSLocalRecovery.create({
      ...
      await flushing;
      const snapshots = await InkDOSLocalRecovery.listSnapshots('recovery-race');
...
def recovery_session_isolation_case(browser, base_url):
    ...
    result = page.evaluate("""async () => {
      ...
      await InkDOSLocalRecovery.clearModule(moduleName);
      let markerA = 'A1', markerB = 'B1';
      const a = InkDOSLocalRecovery.create({
        module: moduleName, sessionId: 'session-a', appVersion: 'session-test',
        serialize: async () => ({marker: markerA})
      });
      const b = InkDOSLocalRecovery.create({
      ...
      const before = await InkDOSLocalRecovery.listSnapshots(moduleName);
      await b.discardCurrent();
      const afterDiscard = await InkDOSLocalRecovery.listSnapshots(moduleName);
      await b.startDocument({documentKey:'same.xlsx', fileName:'same.xlsx', resetSnapshots:true});
      markerB = 'B2'; b.markDirty(); await b.flush();
      const afterRestart = await InkDOSLocalRecovery.listSnapshots(moduleName);
      await b.clearSnapshots();
      const afterClear = await InkDOSLocalRecovery.listSnapshots(moduleName);
...
def recovery_prompt_startup_race_case(browser, base_url):
    ...
    result = page.evaluate("""async () => {
      ...
      await InkDOSLocalRecovery.clearModule(moduleName);
      const seed = InkDOSLocalRecovery.create({
      ...
      const promptAfterDocumentChange = Boolean(document.querySelector('.inkdos-recovery-overlay'));
      const snapshotsAfterDocumentChange = await InkDOSLocalRecovery.listSnapshots(moduleName);

      const pendingCancelledPrompt = manager.promptLatest();
      manager.cancelPrompt();
      await pendingCancelledPrompt;
      await new Promise(resolve => setTimeout(resolve, 0));
      const promptAfterExplicitCancel = Boolean(document.querySelector('.inkdos-recovery-overlay'));
      const snapshotsAfterCancel = await InkDOSLocalRecovery.listSnapshots(moduleName);
      manager.destroy();
      await InkDOSLocalRecovery.clearModule(moduleName);
      return {
...
def recovery_source_rehydration_case(browser, base_url):
    ...
    setup = page.evaluate("""async () => {
      ...
      const inspector = InkDOSLocalRecovery.create({
        module: moduleName,
        appVersion: 'source-rehydrate-test',
      ...
      const db1 = await InkDOSLocalRecovery.openDatabase();
      ...
      const db2 = await InkDOSLocalRecovery.openDatabase();
      ...
      window.__rehydrationManager = InkDOSLocalRecovery.create({
        module: moduleName,
        appVersion: 'source-rehydrate-test',
        serialize: async () => null,
      ...
      return {
        ...
        snapshots: (await InkDOSLocalRecovery.listSnapshots(moduleName)).length
      };
    }""")
    page.wait_for_selector(".inkdos-recovery-overlay", state="visible", timeout=10000)
    page.get_by_role("button", name="Restore", exact=True).click()
    ...
    restored = page.evaluate("""async () => {
      ...
      await InkDOSLocalRecovery.clearModule('recovery-source-rehydrate');
      return result;
    }""")
    page.close()
    context.close()
...
def presentations_case(browser, base_url):
    ...
    page.evaluate("window.__InkDOSPresentationsRecovery.manager.flush()")
    snapshots = page.evaluate("InkDOSLocalRecovery.listSnapshots('presentations').then(items=>items.length)")
    if snapshots < 1:
    ...
    restored.evaluate("InkDOSLocalRecovery.clearModule('presentations')")
    restored.close()
    context.close()
