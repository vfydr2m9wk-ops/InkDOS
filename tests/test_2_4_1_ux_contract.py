...
def test_2_4_1_is_the_single_source_release_version():
...
def test_transient_toolbar_panels_use_one_exclusive_channel():
    ...
    assert "close({restoreFocus:false})" in frame or "close({ restoreFocus: false })" in frame
...
if __name__ == "__main__":
    ...
    for test in tests:
        test()
    print(f"InkDOS 2.4.1 UX contract: {len(tests)} checks passed")
