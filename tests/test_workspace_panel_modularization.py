...
class WorkspacePanelModularizationTests(unittest.TestCase):
    ...
    def test_controller_is_precached_and_documents_direct_load_is_ordered(self):
        ...
        controller_tag = "workspace-panel-controller.js?v=0.20.2.15"
        layout_tag = "workspace-layout.js?v=0.20.2.15"
    ...
    def test_architecture_ratchet_shrinks_workspace_layout_debt(self):
        ...
        self.assertEqual(policy["release"], "0.20.2.15")
        self.assertLessEqual(len(controller.read_text(encoding="utf-8").splitlines()), 500)
        self.assertLess(len(layout.read_text(encoding="utf-8").splitlines()), 1270)
        debt = policy["grandfatheredDebt"]["shared/ui/workspace-layout.js"]
        self.assertEqual(
            debt["maxLines"],
            len(layout.read_text(encoding="utf-8").splitlines()),
        )
