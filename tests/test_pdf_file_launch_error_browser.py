...
visible error instead of failing silently (inkdos:file-launch-error)."""
        ...
        with sync_playwright() as pw:
            ...
            for path in ('picker','launchQueue'):
                ...
                if path=='picker':
                    ...
                    page.evaluate("""()=>window.__launchConsumer({files:[{kind:'file',name:'fake.pdf',getFile:async()=>new File(['not a pdf'],'fake.pdf',{type:'application/pdf'})}]})""")
                page.wait_for_function(VISIBLE,timeout=5000)
                text=page.evaluate(VISIBLE)
                assert 'could not' in text.lower() or 'unsupported' in text.lower() or 'not a pdf' in text.lower(),(path,text)
                ctx.close()
            browser.close()
    finally:
        server.terminate();server.wait(timeout=5)
    print(f'PDF file-launch error visibility ({browser_name}): OK')
