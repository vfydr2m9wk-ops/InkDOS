...
class PdfjsPageLayers{
  ...
  async onPageRendered({pageNumber,shell,record,policy}){if(!this.doc||!record?.page||!shell)return;const page=record.page,viewport=page.getViewport({scale:policy.scale,rotation:policy.rotation});shell.style.setProperty('--scale-factor',String(policy.scale));let m=this.mounts.get(pageNumber);
    if(m&&m.shell===shell){m.viewport=viewport;m.scale=policy.scale;m.rotation=policy.rotation||0;m.annotationLayer.update({viewport});this.editor.updateLayer(pageNumber,viewport);await this.renderText(m);this.applyInteraction(m);return}
    ...
    m={pageNumber,shell,page,scale:policy.scale,rotation:policy.rotation||0,viewport,text,annDiv,editorDiv,annotationLayer};this.mounts.set(pageNumber,m);this.editor.createLayer({pageNumber,div:editorDiv,annotationLayer,viewport});await this.renderText(m);this.applyInteraction(m)}
  ...
  inspect(){return Object.freeze({pages:Object.freeze([...this.mounts.keys()].sort((a,b)=>a-b)),forms:Object.freeze([...this.mounts.values()].map(m=>m.annDiv.querySelectorAll('input,textarea,select').length)),rotations:Object.freeze([...this.mounts.values()].map(m=>m.rotation||0)),interaction:Object.freeze({...this.interaction})})}
}
