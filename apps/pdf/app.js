...
function loadScript(src,test){if(test?.())return Promise.resolve();return new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.onload=()=>test?.()?resolve():resolve();s.onerror=()=>reject(new Error('PDF_LOCAL_ASSET_LOAD_FAILED: '+src));document.head.appendChild(s)})}
...
async function boot(){await loadReaderTools();await loadPageTools();await loadStabilityInfrastructure();NS.ToolbarRail.install($('editbar'));NS.PdfWorker.configure();
...
fileOpen.install();zoomControls.install();navigation.install();extensions.install();modeController.install();readerTools.install();pageTools.install();commands.install();
