...
function create({registry,modeController}={}){const bound=[],guards=[];let unsubscribe=null;
 ...
 function install(){register();bind();unsubscribe=modeController.subscribe(()=>registry.sync());registry.sync()}
 function destroy(){unsubscribe?.();for(const[el,fn]of guards)el.removeEventListener('pointerdown',fn);for(const el of bound)registry.unbindElement(el);for(const id of Object.values(COMMANDS))registry.unregister(id);bound.length=0;guards.length=0}
 return Object.freeze({install,destroy,inspect:()=>Object.freeze({commands:{...COMMANDS},bound:bound.map(el=>el.id||el.dataset.command)})})
}
