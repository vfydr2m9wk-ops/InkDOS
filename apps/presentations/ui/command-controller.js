...
function create({session,history,selection,chrome,fileOpen,save,editor,panel,slideshow,onStructureChange,isBusy=()=>false,onCancelBusy=()=>{}}={}){
 ...
 function execute(id,...args){const command=registry.get(id);if(!command)throw new Error('PRESENTATIONS_COMMAND_NOT_REGISTERED: '+id);if(isBusy()){if(id==='file.new')onCancelBusy();else return false}if(command.isEnabled()===false)return false;return command.handler(...args)}
