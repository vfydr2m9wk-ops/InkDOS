...
let dirty=false,documentActive=false,history=[],historyIndex=-1,historyTimer=null,restoring=false,savedRange=null,currentPage=1,saveReadyUrl='',lastPreparedExport=null;
const lifecycle=window.InkDeskFileLifecycle.create({onChange(value){dirty=value.shouldWarnBeforeUnload;$('dirtyDot').classList.toggle('visible',dirty);document.title=displayedFileName()+(dirty?' •':'')+' — Documents';}});
...
function setDirty(v=true){return v?lifecycle.markDirty():lifecycle.sourceOpened()}
...
async function openFile(file){
 if(!file)return;if(!/\.docx$/i.test(file.name)){alert('This document editor currently supports DOCX files.');fileInput.value='';return}
 if(dirty&&!confirm('This document has changes that are not verified as exported. Discard them and open another file?')){fileInput.value='';return}
 const requestedExport=lifecycle.snapshot().lastExport;
 ...
 try{
  ...
  const nextBuffer=await file.arrayBuffer(),openedSha256=await InkDeskRuntime.sha256Hex(nextBuffer);parsed=await LocalDocxParser.parse(nextBuffer);
  ...
  setTitleValue(file.name);welcome.classList.add('hidden');pagesHost.innerHTML='';await paginate(parsed.blocks,currentPageSpec);buildOutline(parsed.outline);buildPageList();bindPageObserver();$('saveBtn').disabled=false;zoom=1;applyZoom();resetHistory();if(requestedExport&&requestedExport.sha256&&openedSha256===requestedExport.sha256&&Number(requestedExport.bytes)===Number(file.size)){lifecycle.verifyReopened({fileName:file.name,bytes:file.size,sha256:openedSha256});status(lifecycle.label)}else{setDirty(false);status(file.name+' opened')}revokeMediaUrls(previous.mediaUrls);parsed=null;
 }catch(e){
...
async function paginate(blocks,pageSpec){
 ...
 for(let i=0;i<blocks.length;i++){
  ...
  const holder=document.createElement('div');holder.dataset.blockIndex=String(i);if(Number.isFinite(block.sourceIndex))holder.dataset.sourceIndex=String(block.sourceIndex);if(Number.isFinite(block.sourceSubIndex))holder.dataset.sourceSubIndex=String(block.sourceSubIndex);if(!window.InkDeskSafeDOM)throw new Error('The safe DOCX DOM builder did not load.');InkDeskSafeDOM.appendSanitizedHtml(holder,block.html);content.appendChild(holder);current.push({index:i,html:block.html,text:block.text,sourceIndex:block.sourceIndex,sourceSubIndex:block.sourceSubIndex});
 ...
 pages.forEach((entry,pi)=>{const spec=entry.spec,page=document.createElement('section');page.className='page';page.dataset.page=String(pi+1);page.dataset.pageLabel=(pi+1)+' / '+pages.length;page._pageSpec=spec;applyPageSpec(page,spec);if(spec.headerHtml||spec.headerText){const header=document.createElement('div');header.className='page-header';header.contentEditable='false';header.style.top=(spec.headerDistancePx||0)+'px';page.style.setProperty('--doc-header-distance',(spec.headerDistancePx||0)+'px');if(spec.headerHtml&&window.InkDeskSafeDOM)InkDeskSafeDOM.appendSanitizedHtml(header,spec.headerHtml);else header.textContent=spec.headerText;page.appendChild(header)}const pc=document.createElement('div');pc.className='page-content';applyContentSpec(pc,spec);entry.items.forEach(b=>{const wrap=document.createElement('div');wrap.dataset.blockIndex=String(b.index);if(Number.isFinite(b.sourceIndex))wrap.dataset.sourceIndex=String(b.sourceIndex);if(Number.isFinite(b.sourceSubIndex))wrap.dataset.sourceSubIndex=String(b.sourceSubIndex);if(!window.InkDeskSafeDOM)throw new Error('The safe DOCX DOM builder did not load.');InkDeskSafeDOM.appendSanitizedHtml(wrap,b.html);pc.appendChild(wrap)});pc.contentEditable='true';pc.spellcheck=true;page.appendChild(pc);if(spec.footerHtml||spec.footerText){const footer=document.createElement('div');footer.className='page-footer';footer.contentEditable='false';footer.style.bottom=(spec.footerDistancePx||0)+'px';page.style.setProperty('--doc-footer-distance',(spec.footerDistancePx||0)+'px');if(spec.footerHtml&&window.InkDeskSafeDOM)InkDeskSafeDOM.appendSanitizedHtml(footer,spec.footerHtml);else footer.textContent=spec.footerText;page.appendChild(footer)}pagesHost.appendChild(page)});currentPage=1;currentPageSpec=pages[0]?pages[0].spec:pageSpec;updateStats()
}
...
function closeSaveCopyPanel(){const panel=$('saveReadyPanel');if(saveReadyUrl){URL.revokeObjectURL(saveReadyUrl);saveReadyUrl=''}lastPreparedExport=null;if(panel)panel.remove()}
function offerSaveCopy(result){
 closeSaveCopyPanel();lastPreparedExport=result;
 const panel=document.createElement('div');panel.id='saveReadyPanel';panel.className='error-overlay';
 panel.innerHTML='<div class="error-card"><div class="word-badge" style="margin:0 auto 14px;width:38px;height:38px">W</div><h2>Export a copy</h2><p>The original file will not be changed.</p><p class="save-copy-note">InkDesk can request a browser download, but cannot verify that the operating system wrote the file. Changes remain protected until the exported copy is reopened successfully.</p><div class="error-actions"><button id="closeSaveReady">Cancel</button><button id="saveCopyDownload" class="save-copy-link" type="button">Request download</button></div></div>';
 document.body.appendChild(panel);const button=panel.querySelector('#saveCopyDownload');panel.querySelector('#closeSaveReady').onclick=closeSaveCopyPanel;button.onclick=async()=>{button.disabled=true;try{const sha256=result.sha256||await InkDeskRuntime.sha256Hex(result.blob),receipt=InkDeskRuntime.requestDownload(result.blob,result.fileName);lifecycle.downloadRequested(Object.assign({},receipt,{sha256}));status(lifecycle.label);setTimeout(closeSaveCopyPanel,15000)}catch(error){lifecycle.exportFailed(error);status(lifecycle.label);button.disabled=false;alert('Could not request the DOCX download: '+error.message)}}
}
async function save(){if(!pagesHost.querySelector('.page-content'))return;try{lifecycle.beginExport();status(lifecycle.label);const result=await LocalDocxWriter.save(pagesHost,currentFileName,currentBuffer,sourceContext);result.sha256=await InkDeskRuntime.sha256Hex(result.blob);offerSaveCopy(result);status('Copy prepared — download not yet requested')}catch(e){console.error(e);lifecycle.exportFailed(e);status(lifecycle.label);alert('Could not create a copy: '+e.message)}}
...
window.addEventListener('beforeunload',e=>{if(lifecycle.shouldWarnBeforeUnload()){e.preventDefault();e.returnValue=''}});window.addEventListener('resize',()=>{if(pages.length&&window.innerWidth<760)fitWidth()});
window.InkDeskWorkspaceOpenFile=openFile;
if(window.InkDeskFileRouter)InkDeskFileRouter.attachWorkspace({extensions:['docx'],openFile});
