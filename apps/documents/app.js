...
function revokeMediaUrls(urls){if(window.InkDOSRuntime)InkDOSRuntime.revokeObjectUrls(Object.values(urls||{}));else Object.values(urls||{}).forEach(u=>URL.revokeObjectURL(u))}
...
async function openFile(file){
 ...
 try{
  ...
  if(window.InkDOSRuntime)InkDOSRuntime.validateInputSize(file.size,file.name);
  ...
  setTitleValue(file.name);welcome.classList.add('hidden');pagesHost.innerHTML='';await paginate(parsed.blocks,currentPageSpec);buildOutline(parsed.outline);buildPageList();bindPageObserver();$('saveBtn').disabled=false;zoom=1;applyZoom();resetHistory();setDirty(false);if(recovery){await recovery.discardCurrent();await recovery.startDocument({documentKey:fileRecoveryKey(file),fileName:file.name,sourceData:nextBuffer,sourceMeta:{kind:'docx'},resetSnapshots:true})}revokeMediaUrls(previous.mediaUrls);status(file.name+' opened');parsed=null;
 }catch(e){
...
async function createBlankDocument(){
 ...
 buildOutline([]);buildPageList();bindPageObserver();$('saveBtn').disabled=false;zoom=1;applyZoom();resetHistory();setDirty(false);if(recovery)await recovery.startDocument({fileName:currentFileName,resetSnapshots:true});updateStats();status('New document created');
...
function showNewDocumentDialog(){
 ...
 panel.querySelector('#confirmNewDocument').addEventListener('click',async()=>{await createBlankDocument();closeNewDocumentDialog()});
}
...
function applyZoom(){if(Math.abs(zoom-1)<.001){pagesHost.style.transform='none';pagesHost.style.marginBottom='0px'}else{pagesHost.style.transform='scale('+zoom+')';pagesHost.style.marginBottom=Math.max(0,(zoom-1)*pagesHost.scrollHeight)+'px'}const percent=Math.round(zoom*100);$('zoomLabel').textContent=percent+'%';const slider=$('zoomSlider');if(slider)slider.value=String(percent)}
...
function offerSaveCopy(result){
 ...
 document.body.appendChild(panel);const a=panel.querySelector('#saveCopyDownload');a.href=saveReadyUrl;a.download=window.InkDOSRuntime?InkDOSRuntime.sanitizeFileName(result.fileName,'Document copy.docx'):result.fileName;
 panel.querySelector('#closeSaveReady').onclick=closeSaveCopyPanel;a.onclick=()=>{status(dirty?'Download requested — DOCX remains protected':'Download requested');setTimeout(closeSaveCopyPanel,15000)}
}
async function save(){if(!pagesHost.querySelector('.page-content'))return;try{status('Preparing copy…');const result=await LocalDocxWriter.save(pagesHost,currentFileName,currentBuffer,sourceContext);if(recovery&&dirty)await recovery.flush();offerSaveCopy(result);status('Copy ready')}catch(e){console.error(e);alert('Could not create a copy: '+e.message);status('Save copy failed')}}
...
recovery=window.InkDOSLocalRecovery?InkDOSLocalRecovery.create({module:'documents',appVersion:'0.20.3.0',defaultFileName:'Untitled.docx',serialize:captureDocumentRecovery,restore:restoreDocumentRecovery,status:message=>{if(message&&/restored|failed/i.test(message))status(message)}}):null;
if(recovery){window.__InkDOSDocumentsRecovery={manager:recovery,capture:captureDocumentRecovery,restore:restoreDocumentRecovery};recovery.promptLatest()}
updateRulerVisual()
fileInput.addEventListener('change',e=>openFile(e.target.files[0]));$('newBtn').addEventListener('click',newDocument);$('newWelcomeBtn').addEventListener('click',newDocument);$('sidebarBtn').onclick=()=>document.querySelector('.workspace').classList.toggle('sidebar-hidden');$('zoomIn').onclick=()=>{zoom=Math.min(1.8,zoom+.1);applyZoom()};$('zoomOut').onclick=()=>{zoom=Math.max(.45,zoom-.1);applyZoom()};$('zoomSlider').oninput=e=>{zoom=Math.max(.45,Math.min(1.8,Number(e.target.value)/100));applyZoom()};$('zoomLabel').onclick=()=>{zoom=1;applyZoom()};$('fitWidth').onclick=fitWidth;$('saveBtn').onclick=save;
