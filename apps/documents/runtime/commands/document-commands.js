...
function create({fileOpen,saveController,editor,navigation}={}){
 ...
 function install(){if(installed)return;installed=true;register('file.new',()=>fileOpen.requestNew());register('file.open',()=>fileOpen.requestOpen());register('file.save',()=>saveController.save());register('file.share',()=>saveController.share());register('edit.undo',()=>editor.restoreHistory(editor.historyIndex-1));register('edit.redo',()=>editor.restoreHistory(editor.historyIndex+1));FORMAT_COMMANDS.forEach(cmd=>register('format.'+cmd,()=>editor.cmd(cmd)));register('format.fontName',value=>editor.cmd('fontName',value));register('format.fontSize',value=>editor.cmd('fontSize',value));register('format.block',value=>editor.cmd('formatBlock',value));register('format.alignment',value=>{if(value)editor.cmd(value)});register('format.lineSpacing',value=>editor.applyLineSpacing(value));register('format.clear',()=>editor.clearFormatting());register('format.copyFormatting',()=>{copiedFormatting=editor.captureFormatting();return !!copiedFormatting});register('format.applyFormatting',()=>editor.applyFormatting(copiedFormatting));register('insert.hyperlink',()=>editor.editHyperlink());register('format.checklist',()=>editor.toggleChecklist());register('edit.alphaList',()=>{editor.cmd('insertOrderedList');const ol=editor.selectionBlock()?.closest('ol');if(ol)ol.style.listStyleType='upper-alpha'});register('insert.table',()=>editor.insertTable());register('insert.row',()=>editor.addRow());register('insert.column',()=>editor.addColumn());register('insert.image',file=>editor.insertImage(file));register('panel.search',()=>navigation.openPanel('searchPanel'))}
