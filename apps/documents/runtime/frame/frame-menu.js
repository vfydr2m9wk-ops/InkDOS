...
function claim(owner){try{document.dispatchEvent(new CustomEvent(TRANSIENT_EVENT,{detail:{owner}}))}catch(_){}}
function bindDrawer({trigger,drawer,backdrop,closeButton,onOpen,onClose,exclusive=true}={}){
 if(!trigger||!drawer||!backdrop)throw new Error('FrameDrawer requires trigger, drawer and backdrop.');
 const owner={kind:'drawer',trigger};
 function open(){if(exclusive)claim(owner);if(typeof onOpen==='function')onOpen();drawer.hidden=false;backdrop.hidden=false;trigger.setAttribute('aria-expanded','true');requestAnimationFrame(()=>closeButton?.focus?.({preventScroll:true}))}
 ...
 if(exclusive)document.addEventListener(TRANSIENT_EVENT,event=>{if(event.detail?.owner!==owner&&!drawer.hidden)close({restoreFocus:false})});
 return Object.freeze({open,close,toggle,get isOpen(){return !drawer.hidden}})
}
function bindPopover({trigger,popover,onOpen,onClose,exclusive=true}={}){
 if(!trigger||!popover)throw new Error('FramePopover requires trigger and popover.');
 const owner={kind:'popover',trigger};
 ...
 function open(){if(exclusive)claim(owner);position();trigger.setAttribute('aria-expanded','true');if(typeof onOpen==='function')onOpen()}
 ...
 if(exclusive)document.addEventListener(TRANSIENT_EVENT,event=>{if(event.detail?.owner!==owner&&!popover.hidden)close()});
