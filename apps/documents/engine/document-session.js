...
class DocumentSession{
 ...
 commitOpen(id,payload){if(!this.isCurrent(id))return false;const {fileName,sourceBuffer,sourceContext,mediaUrls,kind}=payload||{};this.documentId++;this.fileName=fileName||'Document.docx';this.sourceBuffer=sourceBuffer||null;this.sourceContext=sourceContext||null;this.mediaUrls=mediaUrls||{};this.active=true;this.kind=kind||(sourceBuffer?'docx':'blank');this.revision=0;this.savedRevision=0;return true}
 ...
 saveSnapshot(){return Object.freeze({documentId:this.documentId,revision:this.revision,fileName:this.fileName,sourceBuffer:this.sourceBuffer,sourceContext:this.sourceContext,kind:this.kind})}
