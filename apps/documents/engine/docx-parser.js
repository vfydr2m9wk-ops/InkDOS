...
async function unzip(buffer){
  if(!NS.PackageReader)throw new Error('Documents package reader is unavailable.');
  const pkg=await NS.PackageReader.open(buffer),map=new Map();
  for(const rec of pkg.ordered)map.set(rec.path,await pkg.read(rec.path));
  Object.defineProperty(map,'package',{value:pkg,enumerable:false});
  return map;
}
...
function xml(bytes,context='DOCX package part'){let text;try{text=decode(bytes)}catch(_){throw new Error(context+' is not valid UTF-8 XML.')}if(/<!DOCTYPE|<!ENTITY/i.test(text))throw new Error(context+' contains a forbidden DTD/entity declaration.');const doc=new DOMParser().parseFromString(text,'application/xml');if(doc.getElementsByTagName('parsererror').length)throw new Error(context+' contains malformed XML.');return doc}
...
function parseRunProps(rPr){
  ...
  const color=first(rPr,'color');if(color&&/^[0-9A-Fa-f]{6}$/.test(val(color)))out.color='#'+val(color);
...
function safeFont(value){return String(value||'').replace(/[\\"'<>;{}]/g,'').slice(0,120)}
function cssRun(props){let css='';if(props.fontFamily)css+="font-family:'"+safeFont(props.fontFamily)+"',Arial,sans-serif;";if(props.fontSizePt)css+='font-size:'+props.fontSizePt+'pt;';if(props.bold)css+='font-weight:700;';if(props.italic)css+='font-style:italic;';if(props.underline)css+='text-decoration:underline;';if(props.color)css+='color:'+props.color+';';return css}
...
function renderRun(run,info,mediaUrls,state){
  const drawing=first(run,'drawing')||first(run,'pict');if(drawing){const blip=first(drawing,'blip'),rid=attr(blip,'embed');if(rid&&mediaUrls[rid]){const layout=NS.DrawingLayout?NS.DrawingLayout.imageLayout(drawing,{paragraphLeftPx:(Number(info.props&&info.props.left)||0)/15}):{style:'',anchored:false};if(layout.anchored)state.anchored=true;state.visible=true;return'<img'+(layout.anchored?' class="docx-anchored-image"':'')+' src="'+mediaUrls[rid]+'" data-docx-rel-id="'+esc(rid)+'"'+(layout.style?' style="'+layout.style+'"':'')+' alt="Embedded image">'}return''}
...
async function parse(buffer){
  ...
  try{
    ...
    const finalSect=Array.from(body.children).reverse().find(x=>x.localName==='sectPr')||null,finalSpec=pageSpecFromSect(finalSect,styles,files,root,rels,mediaUrls);for(let j=sectionStart;j<blocks.length;j++)blocks[j].pageSpec=finalSpec;if(sectionStart<blocks.length)blocks[sectionStart].sectionStart=true;if(blocks.length)blocks[0].sectionStart=false;
    ...
    Object.values(mediaUrls).forEach(url=>{try{URL.revokeObjectURL(url)}catch(_){}});
    throw error;
  }
}
NS.DocxParser={parse};
