...
function create({session,chooseFile}={}){const $=id=>document.getElementById(id);let retryChoose=chooseFile;
 ...
 function errorPanel(error,file,options={}){let p=$('openErrorPanel');if(!p){p=document.createElement('div');p.id='openErrorPanel';p.className='error-overlay';p.innerHTML='<div class="error-card"><h2>Document could not be opened</h2><p id="openErrorMessage"></p><div class="error-details"><span>File</span><code id="openErrorFile"></code><span>Engine</span><code>Documents 2.0 private reader</code></div><div class="error-actions"><button id="dismissError">Close</button><button id="retryOpen" class="retry-open">Choose another file</button></div></div>';document.body.appendChild(p);p.querySelector('#dismissError').onclick=()=>p.classList.add('hidden');p.querySelector('#retryOpen').onclick=()=>{p.classList.add('hidden');retryChoose?.()}}p.querySelector('h2').textContent=options.title||'Document could not be opened';p.querySelector('#retryOpen').hidden=options.retry===false;p.querySelector('#openErrorMessage').textContent=error?.message||String(error);p.querySelector('#openErrorFile').textContent=file?.name||'Unknown';p.classList.remove('hidden')}
