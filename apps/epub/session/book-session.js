...
function plainNotes(v,book){return Array.from(S.normalize({notes:v}).notes||[],n=>({...n})).filter(n=>!book||locatorExists(book,n))}
function noteStamp(n){const t=Date.parse(n&&n.updatedAt||n&&n.createdAt||'');return Number.isFinite(t)?t:0}
function mergeNotes(embedded,saved,book){const map=new Map();for(const n of [...plainNotes(embedded,book),...plainNotes(saved,book)]){const prev=map.get(n.id);if(!prev||noteStamp(n)>=noteStamp(prev))map.set(n.id,n)}return Array.from(map.values()).slice(0,S.MAX_NOTES)}
function noteSignature(v){return JSON.stringify(Array.from(v||[],n=>[n.id,n.anchor,n.start,n.end,n.source,n.bookmarkId,n.quote,n.text,n.updatedAt]))}
function create(){
 const state={pkg:null,book:null,fileName:'EPUB Reader',flow:'pages',fontPx:18,fontStyle:'book',theme:'paper',metrics:null,epoch:0,locator:null,pageCount:1,pageIndex:0,operation:0,abort:null,bookKey:null,annotations:[],bookmarks:[],notes:[],selection:null,annotationRevision:0,annotationDirty:false,exportBlob:null,exportReady:false,exportToken:0,preparing:false,notice:'',noticeTimer:null};
 async function loadCandidate(file){
  ...
  const locator=saved&&locatorExists(book,saved.locator)?saved.locator:first,bookmarks=plainBookmarks(saved&&saved.bookmarks,book),embeddedNotes=plainNotes(embedded.notes,book),notes=mergeNotes(embeddedNotes,saved&&saved.notes,book),notesDirty=noteSignature(notes)!==noteSignature(embeddedNotes);
  return {operation,abort,pkg,book,fileName:file.name||book.title||'Book.epub',annotations:plainAnnotations(embedded.highlights),bookmarks,notes,notesDirty,bookKey,saved,flow,fontPx,fontStyle,theme,locator,resumed:!!(saved&&locatorExists(book,saved.locator))};
 }
 ...
 function addNote(input){if(state.notes.length>=S.MAX_NOTES)return null;const n=S.note(input);if(!n||!locatorExists(state.book,n))return null;state.notes.push({...n});state.annotationRevision++;state.annotationDirty=true;state.selection=null;return {...n}}
 function updateNote(id,text){const at=state.notes.findIndex(n=>n.id===id);if(at<0)return null;const previous=state.notes[at],next=S.note({...previous,text,updatedAt:new Date().toISOString()});if(!next||!locatorExists(state.book,next))return null;state.notes[at]={...next};state.annotationRevision++;state.annotationDirty=true;return {...next}}
 function removeNote(id){const at=state.notes.findIndex(n=>n.id===id);if(at<0)return false;state.notes.splice(at,1);state.annotationRevision++;state.annotationDirty=true;return true}
 ...
 async function prepareExport(){
  ...
  try{let blob;if(!state.annotationDirty)blob=new Blob([state.pkg.sourceBytes],{type:'application/epub+zip'});else{blob=await NS.EpubWriter.build(state.pkg,state.annotations,{signal:state.abort&&state.abort.signal,notes:state.notes});const check=await NS.PackageReader.open(await blob.arrayBuffer(),{signal:state.abort&&state.abort.signal}),embedded=await S.readEmbedded(check);if(embedded.highlights.length!==state.annotations.length||embedded.notes.length!==state.notes.length)throw new Error('Annotation verification mismatch')}
 ...
 return Object.freeze({state,loadCandidate,isCurrent,commit,setSelection,addHighlight,removeHighlight,addNote,updateNote,removeNote,toggleBookmark,removeBookmark,editedName,prepareExport,persist,plainAnnotations,plainBookmarks,plainNotes,locatorExists});
}
