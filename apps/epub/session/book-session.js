...
function create(){
 const state={pkg:null,book:null,fileName:'EPUB Reader',flow:'pages',fontPx:18,fontStyle:'book',theme:'paper',metrics:null,epoch:0,locator:null,pageCount:1,pageIndex:0,operation:0,abort:null,bookKey:null,annotations:[],selection:null,annotationRevision:0,annotationDirty:false,exportBlob:null,exportReady:false,exportToken:0,preparing:false,notice:'',noticeTimer:null};
 async function loadCandidate(file){
  ...
  const locator=saved&&locatorExists(book,saved.locator)?saved.locator:first;
  return {operation,abort,pkg,book,fileName:file.name||book.title||'Book.epub',annotations:plainAnnotations(embedded.highlights),bookKey,saved,flow,fontPx,fontStyle,theme,locator,resumed:!!(saved&&locatorExists(book,saved.locator))};
 }
 ...
 return Object.freeze({state,loadCandidate,isCurrent,commit,setSelection,addHighlight,removeHighlight,editedName,prepareExport,persist,plainAnnotations,locatorExists});
}
