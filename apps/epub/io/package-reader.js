...
function hasZip64Extra(bytes,start,length){let p=start,end=start+length;while(p+4<=end){const id=bytes[p]|bytes[p+1]<<8;const n=bytes[p+2]|bytes[p+3]<<8;p+=4;if(p+n>end)fail('invalid-zip','Malformed ZIP extra field');if(id===0x0001)return true;p+=n}return false}
...
async function open(buffer,options={}){
  ...
  if(count===0xffff||centralSize===0xffffffff||centralOffset===0xffffffff)fail('zip64','ZIP64 EPUB is outside this initial scope');
  ...
  for(let i=0;i<count;i++){abort(signal);if(p+46>eocd||u32(view,p)!==0x02014b50)fail('invalid-zip','Invalid ZIP central directory entry');
    ...
    if(flags&1)fail('encrypted','Encrypted/DRM ZIP entries are not supported');if(flags&0x40)fail('encrypted','Strong-encryption ZIP entries are not supported');if(diskStart)fail('multidisk','Multi-disk entry is not supported');
    if(compSize===0xffffffff||size===0xffffffff||localOffset===0xffffffff||hasZip64Extra(bytes,p+46+nameLen,extraLen))fail('zip64','ZIP64 entry is outside this initial scope');
