(function(){'use strict';
const formulaSafety=InkDeskSpreadsheetFormulaSafety.create({
 editor:()=>window.InkDeskSpreadsheetFormulaEditor
});
...
let book=LocalXLSX.createBlank(),active={r:0,c:0},anchor={r:0,c:0},range={r1:0,c1:0,r2:0,c2:0},dirty=false,pendingBlob=null,zoom=100,viewMode='grid',showGridlines=true,editingCell=null,undoStack=[],redoStack=[],dragSelecting=false,dragSelectionPointerId=null,dragSelectionLastKey='',recovery=null,formulaRecoverySyncSuspended=false;
...
async function captureSpreadsheetRecovery(){
 ...
 return{kind:'spreadsheets',schemaVersion:2,fileName:book.fileName||'Untitled.xlsx',book:{active:book.active||0,loaded:true,legacy:Boolean(book.legacy),legacyFormat:book.legacyFormat||'',saveWarning:book.saveWarning||'',sheets:(book.sheets||[]).map(captureSheetRecovery)},selection:{active:{...active},anchor:{...anchor},range:{...range}},view:{zoom,viewMode,showGridlines},formulaDrafts};
}
...
async function restoreSpreadsheetRecovery(context){
 ...
 formulaRecoverySyncSuspended=true;try{
  ...
  recalculateWorkbook();undoStack=[];redoStack=[];refreshUndoState();renderAll();applyViewClasses();applyZoom();
...
recovery=window.InkDeskLocalRecovery?InkDeskLocalRecovery.create({module:'spreadsheets',appVersion:'0.20.2.23',defaultFileName:'Untitled.xlsx',serialize:captureSpreadsheetRecovery,restore:restoreSpreadsheetRecovery,status:message=>{if(message&&/failed/i.test(message))toast(message)}}):null;
