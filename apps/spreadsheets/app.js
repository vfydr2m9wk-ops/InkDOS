(function(){'use strict';
const formulaSafety=InkDOSSpreadsheetFormulaSafety.create({editor:()=>window.InkDOSSpreadsheetFormulaEditor});
const history=InkDOSSpreadsheetHistory.create({limit:80});
...
let book=LocalXLSX.createBlank(),active={r:0,c:0},anchor={r:0,c:0},range={r1:0,c1:0,r2:0,c2:0},dirty=false,pendingBlob=null,zoom=100,viewMode='grid',showGridlines=true,editingCell=null,dragSelecting=false,dragSelectionPointerId=null,dragSelectionLastKey='',recovery=null,formulaRecoverySyncSuspended=false;
function revokeWorkbookObjectUrls(workbook){const urls=new Set();for(const sheet of workbook?.sheets||[])for(const drawing of sheet.drawings||[])if(typeof drawing?.url==='string'&&drawing.url.startsWith('blob:'))urls.add(drawing.url);if(window.InkDOSRuntime)InkDOSRuntime.revokeObjectUrls(urls);else for(const url of urls){try{URL.revokeObjectURL(url)}catch(error){console.warn('Could not revoke workbook object URL',error)}}}
...
function updateWorkbookDocumentTitle(){const name=(book&&book.fileName)||'Untitled.xlsx';document.title=name+(formulaSafety.hasUnsaved(dirty)?' •':'')+' — Spreadsheets'}
function normalizeWorkbookName(name){name=String(name||'').trim()||'Untitled.xlsx';name=name.replace(/\.(?:xls|xlsx)$/i,'');return (name||'Untitled')+'.xlsx'}
function setWorkbookTitleValue(){if(E.title&&document.activeElement!==E.title)E.title.value=(book&&book.fileName)||'Untitled.xlsx';updateWorkbookDocumentTitle()}
function commitWorkbookRename(){if(!book.loaded){E.title.value='Untitled.xlsx';return}const raw=String(E.title.value||'').trim();if(raw===(book.fileName||'')){setWorkbookTitleValue();return}const name=normalizeWorkbookName(raw);if(name!==book.fileName){book.fileName=name;markDirty(true);toast('Workbook renamed to '+name)}setWorkbookTitleValue()}
function markDirty(v=true){dirty=v;if(v){pendingBlob=null;if(recovery)recovery.markDirty()}E.dirty.hidden=!v;updateWorkbookDocumentTitle()}
function workbookRecoveryKey(file){return 'file:'+[file.name,file.size,file.lastModified||0].join(':')}
function jsonClone(value){return value==null?value:JSON.parse(JSON.stringify(value))}
function captureSheetRecovery(source){
 const metadata={};for(const [key,value] of Object.entries(source||{})){
  if(['cells','originalCells','xml','drawings','tables'].includes(key))continue;
  if(value instanceof Map||typeof value==='function')continue;
  try{metadata[key]=jsonClone(value)}catch(_){/* Non-serializable preservation metadata stays in the source package. */}
 }
 return{metadata,cells:Array.from((source&&source.cells)||new Map()).map(([key,value])=>[key,jsonClone(value)])};
}
async function captureSpreadsheetRecovery(){
 if(!book||!book.loaded)return null;
 const editor=window.InkDOSSpreadsheetFormulaEditor,formulaDrafts=editor&&typeof editor.snapshotDrafts==='function'?editor.snapshotDrafts():[];
 return{kind:'spreadsheets',schemaVersion:3,fileName:book.fileName||'Untitled.xlsx',book:{active:book.active||0,loaded:true,legacy:Boolean(book.legacy),legacyFormat:book.legacyFormat||'',saveWarning:book.saveWarning||'',sheets:(book.sheets||[]).map(captureSheetRecovery)},selection:{active:{...active},anchor:{...anchor},range:{...range}},view:{zoom,viewMode,showGridlines},formulaDrafts,history:history.exportState()};
}
function restoreSheetRecovery(base,snapshot){
 const target=base||LocalXLSX.createBlank().sheets[0],metadata=(snapshot&&snapshot.metadata)||{};
 for(const [key,value] of Object.entries(metadata))target[key]=jsonClone(value);
 target.cells=new Map(((snapshot&&snapshot.cells)||[]).map(([key,value])=>[key,jsonClone(value)]));
 target.originalCells=target.originalCells instanceof Map?target.originalCells:new Map();
 target.merges=Array.isArray(target.merges)?target.merges:[];target.widths=target.widths||{};target.heights=target.heights||{};target.drawings=Array.isArray(target.drawings)?target.drawings:[];target.tables=Array.isArray(target.tables)?target.tables:[];
 return target;
}
async function restoreSpreadsheetRecovery(context){
 const payload=context&&context.snapshot&&context.snapshot.payload;if(!payload||payload.kind!=='spreadsheets')throw new Error('Unsupported spreadsheet recovery snapshot.');
 let base=LocalXLSX.createBlank();
 if(context.source&&context.source.data){const meta=context.source.meta||{},legacy=meta.kind==='xls';base=legacy?await LocalXLS.parseWorkbook(context.source.data,payload.fileName):await LocalXLSX.parseWorkbook(context.source.data,payload.fileName)}
 formulaRecoverySyncSuspended=true;try{
  formulaSafety.reset();revokeWorkbookObjectUrls(book);const snapBook=payload.book||{};const baseSheets=base.sheets||[];
  base.sheets=(snapBook.sheets||[]).map((snapshot,index)=>{const metadata=snapshot?.metadata||{},matched=metadata.path?baseSheets.find(item=>item.path===metadata.path):null,byName=!matched&&metadata.name?baseSheets.find(item=>item.name===metadata.name):null;return restoreSheetRecovery(matched||byName||baseSheets[index],snapshot)});if(!base.sheets.length)base.sheets=[restoreSheetRecovery(LocalXLSX.createBlank().sheets[0],{metadata:{name:'Sheet1',maxR:99,maxC:25},cells:[]})];
  base.active=Math.max(0,Math.min(Number(snapBook.active)||0,base.sheets.length-1));base.fileName=payload.fileName||'Recovered.xlsx';base.loaded=true;base.legacy=Boolean(snapBook.legacy);base.legacyFormat=snapBook.legacyFormat||base.legacyFormat;base.saveWarning=snapBook.saveWarning||base.saveWarning;book=base;
  const selection=payload.selection||{};active=selection.active||{r:0,c:0};anchor=selection.anchor||{...active};range=selection.range||{r1:active.r,c1:active.c,r2:active.r,c2:active.c};const view=payload.view||{};zoom=Math.max(50,Math.min(200,Number(view.zoom)||100));viewMode=view.viewMode==='form'?'form':'grid';showGridlines=view.showGridlines!==false;
  recalculateWorkbook();history.importState(payload.history||{});refreshUndoState();renderAll();applyViewClasses();applyZoom();
  const editor=window.InkDOSSpreadsheetFormulaEditor;if(editor&&typeof editor.restoreDrafts==='function')editor.restoreDrafts(payload.formulaDrafts||[]);
 }finally{formulaRecoverySyncSuspended=false}
 markDirty(true);toast((payload.formulaDrafts||[]).length?'Unsaved workbook and formula draft restored from this browser':'Unsaved workbook restored from this browser');
}
...
const formulaIntegrity=window.InkDOSSpreadsheetFormulaIntegrity;
...
function evaluateFormula(currentSheet,formula,stack=new Set(),env={}){
  ...
  if(/^[0-9+\-*/%^(). %]+$/.test(arithmetic)){try{return InkDOSFormula.evaluateArithmetic(arithmetic)}catch(error){console.warn('Arithmetic formula could not be evaluated safely.',error)}}
...
function gridCellAtPoint(clientX,clientY){const hit=document.elementFromPoint?.(clientX,clientY);const cell=hit?.closest?.('.cell');if(!cell||!E.grid.contains(cell))return null;const r=Number(cell.dataset.r),c=Number(cell.dataset.c);return Number.isFinite(r)&&Number.isFinite(c)?{r,c,key:r+':'+c}:null}
function updateDragSelection(point){if(!dragSelecting||!point||point.key===dragSelectionLastKey)return;dragSelectionLastKey=point.key;active={r:point.r,c:point.c};normalizeRange();refreshSelection()}
function cellDown(e){if(e.button!==undefined&&e.button!==0)return;const r=+e.currentTarget.dataset.r,c=+e.currentTarget.dataset.c;if(e.shiftKey)active={r,c};else active=anchor={r,c};normalizeRange();refreshSelection();E.viewport.focus();dragSelecting=true;dragSelectionPointerId=e.pointerId;dragSelectionLastKey=r+':'+c;document.body.classList.add('drag-selecting');e.preventDefault()}
function cellEnter(e){if(!dragSelecting)return;updateDragSelection({r:+e.currentTarget.dataset.r,c:+e.currentTarget.dataset.c,key:e.currentTarget.dataset.r+':'+e.currentTarget.dataset.c})}
function moveDragSelection(e){if(!dragSelecting)return;if(dragSelectionPointerId!==null&&e.pointerId!==undefined&&e.pointerId!==dragSelectionPointerId)return;updateDragSelection(gridCellAtPoint(e.clientX,e.clientY))}
function endDragSelection(e){if(e&&dragSelectionPointerId!==null&&e.pointerId!==undefined&&e.pointerId!==dragSelectionPointerId)return;dragSelecting=false;dragSelectionPointerId=null;dragSelectionLastKey='';document.body.classList.remove('drag-selecting')}
...
async function openFile(file){if(!file)return;if(formulaSafety.hasUnsaved(dirty)&&!confirm('This workbook has unsaved changes. Discard them and open another document?')){E.file.value='';return}if(!/\.xlsx?$/i.test(file.name)){alert('Choose an Excel XLS or XLSX workbook.');return}if(recovery)recovery.cancelPrompt();try{setLoading(true);if(window.InkDOSRuntime)InkDOSRuntime.validateInputSize(file.size,file.name);const buf=await file.arrayBuffer(),legacy=/\.xls$/i.test(file.name)||new Uint8Array(buf,0,Math.min(8,buf.byteLength)).every((v,i)=>v===[0xD0,0xCF,0x11,0xE0,0xA1,0xB1,0x1A,0xE1][i]);const nextBook=legacy?await LocalXLS.parseWorkbook(buf,file.name):await LocalXLSX.parseWorkbook(buf,file.name);if(recovery)await recovery.discardCurrent();formulaSafety.reset();revokeWorkbookObjectUrls(book);book=nextBook;recalculateWorkbook();history.reset();refreshUndoState();active=anchor={r:0,c:0};normalizeRange();renderAll();markDirty(false);if(recovery)await recovery.startDocument({documentKey:workbookRecoveryKey(file),fileName:file.name,sourceData:buf,sourceMeta:{kind:legacy?'xls':'xlsx'},resetSnapshots:true});if(book.legacy){const d=book.legacyDiagnostics||{};toast(`${book.sheets.length} legacy worksheet${book.sheets.length===1?'':'s'} imported · ${d.imageCount||0} image${d.imageCount===1?'':'s'} · Save creates XLSX`)}else{const n=(book.images||[]).filter(x=>x&&x.valid!==false).length,diag=book.imageDiagnostics||{};toast(`${book.sheets.length} worksheet${book.sheets.length===1?'':'s'} opened · ${n} validated image${n===1?'':'s'}${diag.candidates>n?` · ${diag.candidates-n} unsupported candidate${diag.candidates-n===1?'':'s'}`:''}`)}}catch(err){console.error(err);alert('The spreadsheet editor could not open this workbook.\n\n'+(err?.message||err))}finally{setLoading(false);E.file.value=''}}
async function prepareSave(){if(!book.loaded)return toast('Create or open a workbook first');if(formulaSafety.guardSave(toast))return;try{setLoading(true,'Preparing XLSX copy…');pendingBlob=await LocalXLSX.saveCopy(book);if(!pendingBlob||pendingBlob.size===0)throw new Error('The generated XLSX copy is empty');if(E.saveMessage)E.saveMessage.textContent=book.legacy?`${book.saveWarning||'Legacy workbook imported.'} Ready to save ${Math.max(1,Math.round(pendingBlob.size/1024))} KB as a new XLSX copy.`:`Ready to save ${Math.max(1,Math.round(pendingBlob.size/1024))} KB as a new XLSX copy.`;E.savePanel.hidden=false;E.savePanel.style.display='flex'}catch(err){console.error(err);pendingBlob=null;alert('Save copy failed.\n\n'+(err?.message||err))}finally{setLoading(false)}}
async function download(){if(!pendingBlob)return toast('Prepare the XLSX copy first');const base=(book.fileName||'Workbook').replace(/\.xlsx?$/i,''),stamp=new Date().toISOString().slice(0,19).replace(/[T:]/g,'-'),fileName=`${base} copy ${stamp}.xlsx`;E.download.disabled=true;try{if(recovery&&formulaSafety.hasUnsaved(dirty))await recovery.flush();if(window.InkDOSRuntime)InkDOSRuntime.requestDownload(pendingBlob,fileName);else{const a=document.createElement('a'),url=URL.createObjectURL(pendingBlob);a.href=url;a.download=fileName;a.rel='noopener';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),15000)}E.savePanel.hidden=true;E.savePanel.style.display='none';pendingBlob=null;toast(formulaSafety.hasUnsaved(dirty)?'Download requested — confirm the XLSX in Downloads before discarding this workbook':'Download requested; confirm the XLSX copy in your downloads')}catch(error){console.error(error);alert('The XLSX copy could not be downloaded.\n\n'+(error?.message||error))}finally{E.download.disabled=false}}
...
function pushAction(action){history.push(action,book.active);refreshUndoState();markDirty(true)}
function refreshUndoState(){if(E.undo)E.undo.disabled=!history.canUndo();if(E.redo)E.redo.disabled=!history.canRedo()}
function restoreCells(entries,useAfter,targetSheet){for(const item of entries){const value=useAfter?item.after:item.before;if(value)targetSheet.cells.set(item.ref,cloneCell(value));else targetSheet.cells.delete(item.ref)}}
function applyHistoryAction(action,useAfter){if(!action)return false;const index=Math.max(0,Math.min(Number(action.sheetIndex)||0,book.sheets.length-1)),target=book.sheets[index];if(!target)return false;if(action.kind==='cells')restoreCells(action.entries||[],useAfter,target);else if(action.kind==='merges')target.merges=[...(useAfter?action.after:action.before||[])];else return false;recalculateWorkbook();renderGrid();refreshSelection();markDirty(true);return true}
function undo(){if(formulaSafety.guardHistory(toast))return;const a=history.undo();if(!a)return;if(!applyHistoryAction(a,false)){history.redo();return}refreshUndoState()}
function redo(){if(formulaSafety.guardHistory(toast))return;const a=history.redo();if(!a)return;if(!applyHistoryAction(a,true)){history.undo();return}refreshUndoState()}
...
async function newWorkbook(){if(formulaSafety.hasUnsaved(dirty)&&!confirm('Discard unsaved changes and create a new workbook?'))return;if(recovery){recovery.cancelPrompt();await recovery.discardCurrent();}formulaSafety.reset();revokeWorkbookObjectUrls(book);book=LocalXLSX.createBlank();book.loaded=true;book.fileName='Untitled.xlsx';book.sheets[0].maxR=99;book.sheets[0].maxC=25;active=anchor={r:0,c:0};normalizeRange();history.reset();E.empty.hidden=true;E.viewport.hidden=false;E.save.disabled=false;E.title.value='Untitled.xlsx';markDirty(false);if(recovery)await recovery.startDocument({fileName:book.fileName,resetSnapshots:true});setView('grid',false);renderTabs();renderGrid();select(0,0,false);refreshUndoState();toast('New workbook created')}
...
function safeBoot(){
  ...
  E.title.value='Untitled.xlsx';
  E.stats.textContent='Ready';setZoom(100);updateWorkbookDocumentTitle();
  pendingBlob=null;
}
...
E.title.addEventListener('focus',e=>e.target.select());E.title.addEventListener('blur',commitWorkbookRename);E.title.addEventListener('keydown',e=>{e.stopPropagation();if(e.key==='Enter'){e.preventDefault();e.target.blur()}else if(e.key==='Escape'){e.preventDefault();e.target.value=(book&&book.fileName)||'Untitled.xlsx';e.target.blur()}});
...
E.gridMode.onclick=()=>setView('grid');E.formMode.onclick=()=>setView('form');E.gridlines.onclick=()=>{showGridlines=!showGridlines;applyViewClasses();toast(showGridlines?'Gridlines shown':'Gridlines hidden')};E.zoomOut.onclick=()=>setZoom(zoom-10);E.zoomIn.onclick=()=>setZoom(zoom+10);E.zoomSlider.oninput=()=>setZoom(+E.zoomSlider.value);E.fitWidth.onclick=fitWidth;document.addEventListener('inkdos:formula-session-change',()=>{updateWorkbookDocumentTitle();if(!recovery||formulaRecoverySyncSuspended)return;if(formulaSafety.hasDrafts()||dirty){recovery.markDirty();return}recovery.clearSnapshots()});window.addEventListener('keydown',keydown);window.addEventListener('beforeunload',e=>{if(!formulaSafety.hasUnsaved(dirty))return;e.preventDefault();e.returnValue=''});window.addEventListener('pagehide',()=>revokeWorkbookObjectUrls(book),{once:true});window.addEventListener('pointermove',moveDragSelection,{passive:true});window.addEventListener('pointerup',endDragSelection);window.addEventListener('pointercancel',endDragSelection);window.addEventListener('blur',endDragSelection);window.addEventListener('copy',copySelection);window.addEventListener('paste',pasteSelection);
document.body.classList.add('sheet-view');applyViewClasses();safeBoot();
recovery=window.InkDOSLocalRecovery?InkDOSLocalRecovery.create({module:'spreadsheets',appVersion:'0.20.3.0',defaultFileName:'Untitled.xlsx',serialize:captureSpreadsheetRecovery,restore:restoreSpreadsheetRecovery,status:message=>{if(message&&/failed/i.test(message))toast(message)}}):null;
if(recovery){window.__InkDOSSpreadsheetsRecovery={manager:recovery,capture:captureSpreadsheetRecovery,restore:restoreSpreadsheetRecovery};recovery.promptLatest()}
})();
