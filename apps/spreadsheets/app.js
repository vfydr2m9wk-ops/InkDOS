...
async function newWorkbook(){if(formulaSafety.hasUnsaved(dirty)&&!confirm('Discard unsaved changes and create a new workbook?'))return;if(recovery)await recovery.discardCurrent();formulaSafety.reset();revokeWorkbookObjectUrls(book);book=LocalXLSX.createBlank();book.loaded=true;book.fileName='Untitled.xlsx';book.sheets[0].maxR=99;book.sheets[0].maxC=25;active=anchor={r:0,c:0};normalizeRange();history.reset();E.empty.hidden=true;E.viewport.hidden=false;E.save.disabled=false;E.title.value='Untitled.xlsx';markDirty(false);if(recovery)recovery.startDocument({fileName:book.fileName,resetSnapshots:true});setView('grid',false);renderTabs();renderGrid();select(0,0,false);refreshUndoState();toast('New workbook created')}
...
recovery=window.InkDeskLocalRecovery?InkDeskLocalRecovery.create({module:'spreadsheets',appVersion:'0.20.2.26',defaultFileName:'Untitled.xlsx',serialize:captureSpreadsheetRecovery,restore:restoreSpreadsheetRecovery,status:message=>{if(message&&/failed/i.test(message))toast(message)}}):null;
