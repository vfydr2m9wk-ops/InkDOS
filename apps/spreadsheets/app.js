...
let book=LocalXLSX.createBlank(),active={r:0,c:0},anchor={r:0,c:0},range={r1:0,c1:0,r2:0,c2:0},dirty=false,pendingBlob=null,pendingSha256=null,zoom=100,viewMode='grid',showGridlines=true,editingCell=null,undoStack=[],redoStack=[],dragSelecting=false;
const lifecycle=window.InkDeskFileLifecycle.create({onChange(value){dirty=value.shouldWarnBeforeUnload;E.dirty.hidden=!dirty;if(E.saveMessage&&value.state!=='clean')E.saveMessage.textContent=value.label;}});
...
function markDirty(v=true){if(v){pendingBlob=null;pendingSha256=null}return v?lifecycle.markDirty():lifecycle.sourceOpened()}
...
function formulaValue(targetSheet,cellRef,stack=new Set(),env={}){
  const identity=targetSheet.name+'!'+cellRef;if(stack.size>=64||stack.has(identity))return'#CYCLE!';const cell=targetSheet.cells.get(cellRef);if(!cell)return 0;if(!cell.f)return cell.v??0;
...
function evaluateFormula(currentSheet,formula,stack=new Set(),env={}){
  let f=String(formula||'').replace(/^=/,'').trim().replace(/^_xlfn\./i,'');if(f.length>4096)return null;const referenceCount=(f.match(/(?:(?:'[^']+'|[A-Za-z0-9_ .-]+)!)?\$?[A-Z]{1,3}\$?\d+/g)||[]).length;if(referenceCount>256)return null;
  ...
  if(/^[0-9+\-*/(). %^]+$/.test(arithmetic)){try{return InkDeskFormula.evaluateArithmetic(arithmetic)}catch(error){console.warn('Arithmetic formula could not be evaluated safely.',error);return null}}
  const n=Number(f.replace(',','.'));return Number.isFinite(n)?n:null;
}
function recalculateWorkbook(){
  let evaluations=0;const maxEvaluations=20000;for(let pass=0;pass<4;pass++)for(const s of book.sheets)for(const[cellRef,cell]of s.cells){if(!cell?.f)continue;if(++evaluations>maxEvaluations){cell.recalculation='limit';continue}const v=evaluateFormula(s,cell.f,new Set([s.name+'!'+cellRef]),{});if(v!==null&&v!==undefined){delete cell.recalculation;cell.calculatedRaw=v;cell.calculated=formatFormulaResult(v);cell.display=String(formatFormulaResult(v))}else{cell.recalculation='unsupported';delete cell.calculated;delete cell.calculatedRaw}}
}
...
async function openFile(file){if(!file)return;const requestedExport=lifecycle.snapshot().lastExport;if(dirty&&!confirm('This workbook has changes that are not verified as exported. Discard them and open another document?')){E.file.value='';return}if(!/\.xlsx?$/i.test(file.name)){alert('Choose an Excel XLS or XLSX workbook.');E.file.value='';return}try{setLoading(true);if(window.InkDeskRuntime)InkDeskRuntime.validateInputSize(file.size,file.name);const buf=await file.arrayBuffer(),openedSha256=await InkDeskRuntime.sha256Hex(buf),legacy=/\.xls$/i.test(file.name)||new Uint8Array(buf,0,Math.min(8,buf.byteLength)).every((v,i)=>v===[0xD0,0xCF,0x11,0xE0,0xA1,0xB1,0x1A,0xE1][i]);const nextBook=legacy?await LocalXLS.parseWorkbook(buf,file.name):await LocalXLSX.parseWorkbook(buf,file.name);revokeWorkbookObjectUrls(book);book=nextBook;recalculateWorkbook();undoStack=[];redoStack=[];refreshUndoState();active=anchor={r:0,c:0};normalizeRange();renderAll();if(requestedExport&&requestedExport.sha256&&openedSha256===requestedExport.sha256&&Number(requestedExport.bytes)===Number(file.size)){lifecycle.verifyReopened({fileName:file.name,bytes:file.size,sha256:openedSha256});toast(lifecycle.label)}else markDirty(false);if(book.legacy){const d=book.legacyDiagnostics||{};toast(`${book.sheets.length} legacy worksheet${book.sheets.length===1?'':'s'} imported · ${d.imageCount||0} image${d.imageCount===1?'':'s'} · Save creates XLSX`)}else{const n=(book.images||[]).filter(x=>x&&x.valid!==false).length,diag=book.imageDiagnostics||{};toast(`${book.sheets.length} worksheet${book.sheets.length===1?'':'s'} opened · ${n} validated image${n===1?'':'s'}${diag.candidates>n?` · ${diag.candidates-n} unsupported candidate${diag.candidates-n===1?'':'s'}`:''}`)}}catch(err){console.error(err);alert('The spreadsheet editor could not open this workbook.\n\n'+(err?.message||err))}finally{setLoading(false);E.file.value=''}}
async function prepareSave(){if(!book.loaded)return toast('Create or open a workbook first');try{lifecycle.beginExport();setLoading(true,lifecycle.label);pendingBlob=await LocalXLSX.saveCopy(book);if(!pendingBlob||pendingBlob.size===0)throw new Error('The generated XLSX copy is empty');pendingSha256=await InkDeskRuntime.sha256Hex(pendingBlob);if(E.saveMessage)E.saveMessage.textContent=book.legacy?`${book.saveWarning||'Legacy workbook imported.'} Copy prepared (${Math.max(1,Math.round(pendingBlob.size/1024))} KB). A browser download cannot be verified until the copy is reopened.`:`Copy prepared (${Math.max(1,Math.round(pendingBlob.size/1024))} KB). A browser download cannot be verified until the copy is reopened.`;E.savePanel.hidden=false;E.savePanel.style.display='flex'}catch(err){console.error(err);pendingBlob=null;pendingSha256=null;lifecycle.exportFailed(err);alert('Export copy failed.\n\n'+(err?.message||err))}finally{setLoading(false)}}
async function download(){if(!pendingBlob)return toast('Prepare the XLSX copy first');const base=(book.fileName||'Workbook').replace(/\.xlsx?$/i,''),stamp=new Date().toISOString().slice(0,19).replace(/[T:]/g,'-'),fileName=`${base} copy ${stamp}.xlsx`;try{const sha256=pendingSha256||await InkDeskRuntime.sha256Hex(pendingBlob),receipt=InkDeskRuntime.requestDownload(pendingBlob,fileName);lifecycle.downloadRequested(Object.assign({},receipt,{sha256}));E.savePanel.hidden=true;E.savePanel.style.display='none';pendingBlob=null;pendingSha256=null;toast(lifecycle.label)}catch(error){console.error(error);lifecycle.exportFailed(error);alert('The XLSX download could not be requested.\n\n'+(error?.message||error))}}
...
function safeBoot(){
  ...
  pendingBlob=null;pendingSha256=null;
}
...
$('closeSaveBtn').onclick=()=>{E.savePanel.hidden=true;E.savePanel.style.display='none';pendingBlob=null;pendingSha256=null};
...
E.gridMode.onclick=()=>setView('grid');E.formMode.onclick=()=>setView('form');E.gridlines.onclick=()=>{showGridlines=!showGridlines;applyViewClasses();toast(showGridlines?'Gridlines shown':'Gridlines hidden')};E.zoomOut.onclick=()=>setZoom(zoom-10);E.zoomIn.onclick=()=>setZoom(zoom+10);E.zoomSlider.oninput=()=>setZoom(+E.zoomSlider.value);E.fitWidth.onclick=fitWidth;window.addEventListener('keydown',keydown);window.addEventListener('beforeunload',e=>{if(!lifecycle.shouldWarnBeforeUnload())return;e.preventDefault();e.returnValue=''});window.addEventListener('pagehide',()=>revokeWorkbookObjectUrls(book),{once:true});window.addEventListener('pointerup',endDragSelection);window.addEventListener('copy',copySelection);window.addEventListener('paste',pasteSelection);
window.InkDeskWorkspaceOpenFile=openFile;
if(window.InkDeskFileRouter)InkDeskFileRouter.attachWorkspace({extensions:['xls','xlsx'],openFile});
