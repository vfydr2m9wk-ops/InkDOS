...
function onCommitted(info){if(!editorController)editorController=NS.EditorController.create({session,chrome,dialog,actions});editorController.activate();chrome.sync();chrome.toast(info.newBook?'New workbook created':`${session.book.sheets.length} worksheet${session.book.sheets.length===1?'':'s'} opened${info.legacy?' · XLS imported as editable XLSX model':''}`)}
