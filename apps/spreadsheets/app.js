(function(){'use strict';
const formulaSafety=InkDeskSpreadsheetFormulaSafety.create({editor:()=>window.InkDeskSpreadsheetFormulaEditor});
const history=InkDeskSpreadsheetHistory.create({limit:80});
...
async function captureSpreadsheetRecovery(){
 ...
 const editor=window.InkDeskSpreadsheetFormulaEditor,formulaDrafts=editor&&typeof editor.snapshotDrafts==='function'?editor.snapshotDrafts():[];
...
async function restoreSpreadsheetRecovery(context){
 ...
 formulaRecoverySyncSuspended=true;try{
  ...
  base.sheets=(snapBook.sheets||[]).map((snapshot,index)=>restoreSheetRecovery(baseSheets[index],snapshot));if(!base.sheets.length)base.sheets=[restoreSheetRecovery(LocalXLSX.createBlank().sheets[0],{metadata:{name:'Sheet1',maxR:99,maxC:25},cells:[]})];
  ...
  const editor=window.InkDeskSpreadsheetFormulaEditor;if(editor&&typeof editor.restoreDrafts==='function')editor.restoreDrafts(payload.formulaDrafts||[]);
...
function renderTabs(){InkDeskSpreadsheetWorksheetTabs.render(E.tabs,book,{createBlank:()=>LocalXLSX.createBlank().sheets[0],canAdd:()=>!formulaSafety.hasDrafts()||(toast('Confirm or cancel formula drafts before adding a worksheet.'),false),onActivate:i=>{book.active=i;active=anchor={r:0,c:0};normalizeRange();renderAll()},onAdd:(name,i)=>{book.active=i;active=anchor={r:0,c:0};normalizeRange();history.reset();refreshUndoState();renderAll();markDirty(true);toast(name+' added')}})}
