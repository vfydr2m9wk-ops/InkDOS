...
function updateWorkbookDocumentTitle(){const name=(book&&book.fileName)||'Untitled.xlsx';document.title=name+(dirty?' •':'')+' — Spreadsheets'}
...
E.gridMode.onclick=()=>setView('grid');E.formMode.onclick=()=>setView('form');E.gridlines.onclick=()=>{showGridlines=!showGridlines;applyViewClasses();toast(showGridlines?'Gridlines shown':'Gridlines hidden')};E.zoomOut.onclick=()=>setZoom(zoom-10);E.zoomIn.onclick=()=>setZoom(zoom+10);E.zoomSlider.oninput=()=>setZoom(+E.zoomSlider.value);E.fitWidth.onclick=fitWidth;window.addEventListener('keydown',keydown);window.addEventListener('beforeunload',e=>{if(!dirty)return;e.preventDefault();e.returnValue=''});window.addEventListener('pagehide',()=>revokeWorkbookObjectUrls(book),{once:true});window.addEventListener('pointermove',moveDragSelection,{passive:true});window.addEventListener('pointerup',endDragSelection);window.addEventListener('pointercancel',endDragSelection);window.addEventListener('blur',endDragSelection);window.addEventListener('copy',copySelection);window.addEventListener('paste',pasteSelection);
document.body.classList.add('sheet-view');applyViewClasses();safeBoot();
recovery=window.InkDeskLocalRecovery?InkDeskLocalRecovery.create({module:'spreadsheets',appVersion:'0.20.2.11',defaultFileName:'Untitled.xlsx',serialize:captureSpreadsheetRecovery,restore:restoreSpreadsheetRecovery,status:message=>{if(message&&/failed/i.test(message))toast(message)}}):null;
