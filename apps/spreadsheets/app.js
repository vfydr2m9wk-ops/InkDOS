...
const NS=root.InkDOS2Spreadsheets=root.InkDOS2Spreadsheets||{},session=new NS.WorkbookSession(),dialog=NS.SessionDialog.create(),chrome=NS.ChromeController.create({session}),actions={};let editorController=null,leaving=false;
...
const home=document.querySelector('a[aria-label="Home"]');home?.addEventListener('click',async e=>{if(leaving||!session.dirty)return;e.preventDefault();leaving=true;try{if(await openController.requestLeave())root.location.href=home.href}finally{leaving=false}});
