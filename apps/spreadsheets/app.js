...
let book=LocalXLSX.createBlank(),active={r:0,c:0},anchor={r:0,c:0},range={r1:0,c1:0,r2:0,c2:0},dirty=false,pendingBlob=null,zoom=100,viewMode='grid',showGridlines=true,editingCell=null,undoStack=[],redoStack=[],dragSelecting=false,dragSelectionPointerId=null,dragSelectionLastKey='',recovery=null;
...
async function captureSpreadsheetRecovery(){
 ...
 return{kind:'spreadsheets',schemaVersion:1,fileName:book.fileName||'Untitled.xlsx',book:{active:book.active||0,loaded:true,legacy:Boolean(book.legacy),legacyFormat:book.legacyFormat||'',saveWarning:book.saveWarning||'',sheets:(book.sheets||[]).map(captureSheetRecovery)},selection:{active:{...active},anchor:{...anchor},range:{...range}},view:{zoom,viewMode,showGridlines}};
}
...
async function restoreSpreadsheetRecovery(context){
 ...
 revokeWorkbookObjectUrls(book);const snapBook=payload.book||{};const baseSheets=base.sheets||[];
 ...
 recalculateWorkbook();undoStack=[];redoStack=[];refreshUndoState();renderAll();applyViewClasses();applyZoom();markDirty(true);toast('Unsaved workbook restored from this browser');
}
