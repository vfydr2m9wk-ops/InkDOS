...
function parseXf(data,fonts,formats,palette){if(data.length<20)return{};const v=bytesView(data),fontId=u16(v,0),numFmtId=u16(v,2),protection=u16(v,4),a=data[6],rotation=data[7],textProps=data[8],used=data[9],b1=u32(v,10),b2=u32(v,14),fill=u16(v,18),fillPattern=(b2>>>26)&0x3F,fg=fill&0x7F,bg=(fill>>>7)&0x7F;const resolvedFontId=fontId===4?0:fontId>4?fontId-1:fontId,side=(style,color)=>style?{style:BORDER_STYLE[style]||'thin',color:colorAt(color,palette)||'#70757d'}:null,border={};const values={left:side(b1&15,(b1>>>16)&0x7F),right:side((b1>>>4)&15,(b1>>>23)&0x7F),top:side((b1>>>8)&15,b2&0x7F),bottom:side((b1>>>12)&15,(b2>>>7)&0x7F)};for(const[k,val]of Object.entries(values))if(val)border[k]=val;const diagonalStyle=(b2>>>21)&15,diagonalColor=(b2>>>14)&0x7F;if(diagonalStyle&&(b2&(1<<30)||b2&(1<<31)))border.diagonal=side(diagonalStyle,diagonalColor);const fmt=formats[numFmtId]??BUILTIN_FORMATS[numFmtId]??'';return{font:fonts[resolvedFontId]||{},fill:fillPattern?colorAt(fg,palette):(bg&&bg!==64?colorAt(bg,palette):''),fillPattern,border,align:H_ALIGN[a&7]||'general',vertical:V_ALIGN[(a>>>4)&7]||'',wrap:!!(a&8),shrinkToFit:!!(textProps&0x10),indent:textProps&0x0F,rotation:rotation>90?rotation-180:rotation,numFmtId,numberFormat:fmt,locked:!!(protection&1),hidden:!!(protection&2),usedAttributes:used,hideZero:String(fmt).split(';').length>=3&&/^\s*(?:""|)\s*$/.test(String(fmt).split(';')[2]||'')}}
...
function parseSheet(records,meta,globalInfo){
  const cells=new Map(),originalCells=new Map(),merges=[],widths={},heights={},styles=globalInfo.styles;let maxR=39,maxC=15,defaultColWidth=68,defaultRowHeight=20,freezePane=null,pageSetup={},margins={},autoFilter='',pendingStringFormula=null,header='',footer='',showZeros=true,showGridlines=true;
  for(let i=0;i<records.length;i++){
    ...
    else if(r.type===0x023E&&d.length>=2){const options=u16(v,0);showGridlines=!!(options&0x0002);showZeros=!!(options&0x0010)}
