...
function create({nameBox,input,functions,session,selection,editor,onNavigate,onCommitted,beforeCommit}={}){
 ...
 input.addEventListener('focus',sync);input.addEventListener('keydown',async e=>{if(e.key==='Enter'&&session.book?.loaded){e.preventDefault();const value=normalizeFormulaInput(input.value);if(await beforeCommit?.(value)===false)return;editor.commitValue(value);onCommitted?.();sync()}else if(e.key==='Escape'){e.preventDefault();sync()}});nameBox.addEventListener('keydown',e=>{if(e.key!=='Enter'||!session.book?.loaded)return;e.preventDefault();const p=root.LocalXLSX.decodeRef(nameBox.value.toUpperCase());selection.select(p.r,p.c,session.activeSheet(),false);onNavigate?.();sync()});functions?.addEventListener('change',()=>{const fn=functions.value;functions.value='';if(!fn)return;const q=selection.range,ref=(r,c)=>root.LocalXLSX.encodeRef(r,c);if(['SUM','AVERAGE','MIN','MAX','COUNT'].includes(fn)){input.value=`=${fn}(${ref(q.r1,q.c1)}:${ref(q.r2,q.c2)})`;input.focus();input.setSelectionRange(input.value.length,input.value.length)}});
