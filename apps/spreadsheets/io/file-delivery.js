...
function safeName(name){let n=String(name||'Untitled.xlsx').trim()||'Untitled.xlsx';n=n.replace(/[\\/:*?"<>|]+/g,'-');return /\.xlsx$/i.test(n)?n:n+'.xlsx'}
function toFile(blob,fileName){try{return new File([blob],safeName(fileName),{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',lastModified:Date.now()})}catch(_){return null}}
...
async function viaPicker(blob,fileName){let handle;try{handle=await global.showSaveFilePicker({suggestedName:safeName(fileName),types:[{description:'Excel workbook',accept:{'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':['.xlsx']}}]})}catch(e){if(e?.name==='AbortError')fail('cancelled','Save cancelled.',e);fail('picker-blocked','Native save picker was blocked by this host.',e)}try{const writable=await handle.createWritable();await writable.write(blob);await writable.close()}catch(e){fail('write-failed','The selected XLSX could not be written.',e)}return Object.freeze({method:'file-system-access',fileName:safeName(fileName),deliveryConfirmed:true})}
...
async function deliver(blob,fileName){const file=toFile(blob,fileName),c=capabilities(blob,fileName);
 ...
 if(c.local){if(c.share)return viaShare(blob,fileName,file);fail('local-delivery-unavailable','This HTML viewer blocks direct XLSX downloads. Use a browser/host with system file sharing enabled, or open the InkDOS app from its normal host.');}
...
NS.FileDelivery=Object.freeze({deliver,capabilities,safeName});
})(globalThis);
