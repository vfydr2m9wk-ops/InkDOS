...
function formulaCached(data){if(data.length<14)return{value:'',type:'s',special:true};const r=data.subarray(6,14),v=bytesView(r);if(r[6]===0xFF&&r[7]===0xFF){const type=r[0];if(type===1)return{value:!!r[2],type:'b',special:true};if(type===2)return{value:'#ERROR!',type:'e',special:true};if(type===3)return{value:'',type:'s',special:true};return{value:'',type:'s',special:true,string:true}}return{value:f64(v,0),type:'n',special:false}}
function parseStringRecord(data){return unicodeString(data,0,2).text}
function formulaRef(rowRaw,colRaw,baseRow,baseCol,relative){
  const rowRel=!!(colRaw&0x8000),colRel=!!(colRaw&0x4000);let row,col;
  if(relative){row=rowRel?baseRow+(rowRaw&0x8000?rowRaw-0x10000:rowRaw):rowRaw;const low=colRaw&0xFF;col=colRel?baseCol+(low&0x80?low-0x100:low):low}
  else{row=rowRaw;col=colRaw&0xFF}
  row=Math.max(0,Math.min(65535,row));col=Math.max(0,Math.min(255,col));const colText=colName(col),rowText=String(row+1);return`${colRel?'':'$'}${colText}${rowRel?'':'$'}${rowText}`
}
function formulaArea(bytes,offset,baseRow,baseCol,relative){const v=bytesView(bytes),r1=u16(v,offset),r2=u16(v,offset+2),c1=u16(v,offset+4),c2=u16(v,offset+6);return`${formulaRef(r1,c1,baseRow,baseCol,relative)}:${formulaRef(r2,c2,baseRow,baseCol,relative)}`}
const FORMULA_FUNCTIONS={0:['COUNT',1],1:['IF',3],4:['SUM',1],5:['AVERAGE',1],6:['MIN',1],7:['MAX',1],15:['SIN',1],16:['COS',1],17:['TAN',1],19:['PI',0],20:['SQRT',1],21:['EXP',1],22:['LN',1],23:['LOG10',1],24:['ABS',1],25:['INT',1],27:['ROUND',2],34:['TRUE',0],35:['FALSE',0],36:['AND',1],37:['OR',1],38:['NOT',1],39:['MOD',2],183:['PRODUCT',1]};
function decodeFormulaTokens(tokens,baseRow,baseCol){const stack=[];let p=0,ok=true;const pop=()=>stack.length?stack.pop():null,push=v=>{if(v==null)ok=false;else stack.push(String(v))},binary=op=>{const b=pop(),a=pop();if(a==null||b==null){ok=false;return}push(`(${a}${op}${b})`)};
  while(ok&&p<tokens.length){const op=tokens[p++];
    if([0x24,0x44,0x64,0x2C,0x4C,0x6C].includes(op)){if(p+4>tokens.length){ok=false;break}const v=bytesView(tokens),row=u16(v,p),col=u16(v,p+2);push(formulaRef(row,col,baseRow,baseCol,[0x2C,0x4C,0x6C].includes(op)));p+=4;continue}
    if([0x25,0x45,0x65,0x2D,0x4D,0x6D].includes(op)){if(p+8>tokens.length){ok=false;break}push(formulaArea(tokens,p,baseRow,baseCol,[0x2D,0x4D,0x6D].includes(op)));p+=8;continue}
    if(op===0x1E){if(p+2>tokens.length){ok=false;break}push(u16(bytesView(tokens),p));p+=2;continue}
    if(op===0x1F){if(p+8>tokens.length){ok=false;break}push(String(f64(bytesView(tokens),p)));p+=8;continue}
    if(op===0x1D){if(p>=tokens.length){ok=false;break}push(tokens[p++]?'TRUE':'FALSE');continue}
    if(op===0x1C){if(p>=tokens.length){ok=false;break}const errors={0:'#NULL!',7:'#DIV/0!',15:'#VALUE!',23:'#REF!',29:'#NAME?',36:'#NUM!',42:'#N/A'};push(errors[tokens[p++]]||'#ERROR!');continue}
    if(op===0x17){if(p+2>tokens.length){ok=false;break}const cch=tokens[p++],flags=tokens[p++],wide=!!(flags&1),count=cch*(wide?2:1);if(p+count>tokens.length){ok=false;break}const raw=tokens.subarray(p,p+count),value=wide?utf16(raw):cp1252(raw);p+=count;push(`"${String(value).replace(/"/g,'""')}"`);continue}
    if(op>=0x03&&op<=0x0E){binary({0x03:'+',0x04:'-',0x05:'*',0x06:'/',0x07:'^',0x08:'&',0x09:'<',0x0A:'<=',0x0B:'=',0x0C:'>=',0x0D:'>',0x0E:'<>'}[op]);continue}
    if(op===0x12||op===0x13){const a=pop();if(a==null){ok=false;break}push(`${op===0x13?'-':'+'}(${a})`);continue}
    if(op===0x14){const a=pop();if(a==null){ok=false;break}push(`(${a})%`);continue}
    if(op===0x15){const a=pop();if(a==null){ok=false;break}push(`(${a})`);continue}
    if(op===0x19){if(p+3>tokens.length){ok=false;break}const flags=tokens[p],data=u16(bytesView(tokens),p+1);p+=3;if(flags&0x10){const a=pop();if(a==null){ok=false;break}push(`SUM(${a})`)}void data;continue}
    if([0x21,0x41,0x61].includes(op)){if(p+2>tokens.length){ok=false;break}const id=u16(bytesView(tokens),p)&0x7FFF;p+=2;const meta=FORMULA_FUNCTIONS[id];if(!meta){ok=false;break}const args=[];for(let i=0;i<meta[1];i++)args.unshift(pop());if(args.some(x=>x==null)){ok=false;break}push(`${meta[0]}(${args.join(',')})`);continue}
    if([0x22,0x42,0x62].includes(op)){if(p+3>tokens.length){ok=false;break}const argc=tokens[p++],id=u16(bytesView(tokens),p)&0x7FFF;p+=2;const meta=FORMULA_FUNCTIONS[id];if(!meta){ok=false;break}const args=[];for(let i=0;i<argc;i++)args.unshift(pop());if(args.some(x=>x==null)){ok=false;break}push(`${meta[0]}(${args.join(',')})`);continue}
    ok=false;
  }
  return ok&&stack.length===1?stack[0]:''
}
function sharedFormulaDefinitions(records){const out=[];for(const r of records){if(r.type!==0x04BC||r.data.length<10)continue;const v=bytesView(r.data),r1=u16(v,0),r2=u16(v,2),c1=r.data[4],c2=r.data[5],cce=u16(v,8);if(10+cce>r.data.length)continue;out.push({r1,r2,c1,c2,tokens:r.data.subarray(10,10+cce)})}return out}
function decodedFormula(data,row,col,shared){if(data.length<22)return'';const cce=u16(bytesView(data),20),tokens=data.subarray(22,Math.min(data.length,22+cce));if(tokens.length===5&&tokens[0]===0x01){const def=shared.find(x=>row>=x.r1&&row<=x.r2&&col>=x.c1&&col<=x.c2);return def?decodeFormulaTokens(def.tokens,row,col):''}return decodeFormulaTokens(tokens,row,col)}
...
function parseSheet(records,meta,globalInfo){
  const cells=new Map(),originalCells=new Map(),merges=[],widths={},heights={},styles=globalInfo.styles,sharedFormulas=sharedFormulaDefinitions(records);let maxR=39,maxC=15,defaultColWidth=68,defaultRowHeight=20,freezePane=null,pageSetup={},margins={},autoFilter='',pendingStringFormula=null,header='',footer='',decodedFormulaCount=0;
  for(let i=0;i<records.length;i++){
    ...
    else if(r.type===0x0006&&d.length>=14){row=u16(v,0);col=u16(v,2);xf=u16(v,4);const cached=formulaCached(d),cellStyle=styles[xf]||{},formula=decodedFormula(d,row,col,sharedFormulas);if(formula)decodedFormulaCount++;cell={v:cached.value,f:formula,styleId:xf,style:cellStyle,t:cached.type,display:cached.type==='n'?formatNumber(cached.value,cellStyle,globalInfo.date1904):cached.type==='b'?(cached.value?'TRUE':'FALSE'):String(cached.value??''),legacyFormula:true,legacyFormulaDecoded:!!formula};if(cached.string)pendingStringFormula={row,col,xf,cell}}
...
async function parseWorkbook(buffer,fileName='Workbook.xls'){
  ...
  if(!sheets.length)throw new Error('The BIFF8 worksheets could not be decoded');active=Math.min(Math.max(0,active),sheets.length-1);if(sheets[active]?.state!=='visible'){const v=sheets.findIndex(s=>s.state==='visible');if(v>=0)active=v}const formulaCount=sheets.reduce((n,s)=>n+s.legacyFormulaCount,0),decodedFormulaCount=sheets.reduce((n,s)=>n+(s.decodedFormulaCount||0),0),drawingCount=sheets.reduce((n,s)=>n+(s.drawings||[]).length,0);
  return{zip:null,sheets,active,fileName,loaded:true,legacy:true,date1904,legacyFormat:'BIFF8',legacyStyles:styles,images,workbookXml:'',legacyDiagnostics:{formulaCount,decodedFormulaCount,drawingCount,imageCount:images.length},saveWarning:`This Excel 97–2003 workbook is imported locally. Saving creates an XLSX copy. ${decodedFormulaCount}/${formulaCount} legacy formulas were recovered; unsupported formula tokens keep their cached displayed result. Macros or unsupported embedded objects are not carried over.`};
}
