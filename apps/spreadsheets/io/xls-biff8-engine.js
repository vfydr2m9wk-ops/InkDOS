...
function parseSheet(records,meta,globalInfo){
  ...
  for(let i=0;i<records.length;i++){
    ...
    else if(r.type===0x0006&&d.length>=14){row=u16(v,0);col=u16(v,2);xf=u16(v,4);const cached=formulaCached(d),cellStyle=styles[xf]||{},formula=decodedFormula(d,row,col,sharedFormulas);if(formula)decodedFormulaCount++;cell={v:cached.value,f:formula,styleId:xf,style:cellStyle,t:typeof cached.value==='string'?'s':'n',display:typeof cached.value==='string'?cached.value:formatNumber(cached.value,cellStyle,globalInfo.date1904),legacyFormula:true,legacyFormulaDecoded:!!formula};if(cached.string)pendingStringFormula={row,col,xf,cell}}
