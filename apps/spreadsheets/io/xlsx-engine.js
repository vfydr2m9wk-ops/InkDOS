...
function xml(text,context='XLSX package part'){
  if(global.InkDOS2SpreadsheetPackageValidator)return global.InkDOS2SpreadsheetPackageValidator.parseXml(text,context);
  if(/<!DOCTYPE\b|<!ENTITY\b/i.test(String(text||'')))throw new Error('Forbidden XML declaration in '+context);
...
function cloneCell(cell){
  if(!cell)return null;
  return{v:cell.v,f:cell.f||'',styleId:+(cell.styleId||0),style:cell.style?JSON.parse(JSON.stringify(cell.style)):{},t:cell.t||'',display:cell.display??'',hyperlink:cell.hyperlink?JSON.parse(JSON.stringify(cell.hyperlink)):null,comment:cell.comment?JSON.parse(JSON.stringify(cell.comment)):null};
}
function sameCell(a,b){
  if(!a&&!b)return true;if(!a||!b)return false;
  return String(a.f||'')===String(b.f||'')&&String(a.t||'')===String(b.t||'')&&Number(a.styleId||0)===Number(b.styleId||0)&&JSON.stringify(a.style||{})===JSON.stringify(b.style||{})&&String(a.v??'')===String(b.v??'')&&JSON.stringify(a.hyperlink||null)===JSON.stringify(b.hyperlink||null)&&JSON.stringify(a.comment||null)===JSON.stringify(b.comment||null);
}
...
async function parseWorkbook(buffer,fileName='Workbook.xlsx'){
  ...
  for(const [sheetIndex,s] of localAll(workbookXml,'sheet').entries()){
    ...
    sheets.push({name,state,path:target,xml:raw,cells,originalCells,merges,originalMerges:[...merges],widths,originalWidths:{...widths},heights,originalHeights:{...heights},defaultColWidth,defaultRowHeight,drawings,tables,maxR:Math.min(maxR+12,600),maxC:Math.min(maxC+6,100),printArea:definedPrintAreas[sheetIndex]||'',margins,pageSetup,freezePane:pane?{xSplit:+(pane.getAttribute('xSplit')||0),ySplit:+(pane.getAttribute('ySplit')||0),topLeftCell:pane.getAttribute('topLeftCell')||'',state:pane.getAttribute('state')||''}:null,autoFilter:filter?.getAttribute('ref')||'',originalAutoFilter:filter?.getAttribute('ref')||'',hasConditionalFormatting:!!localOne(doc,'conditionalFormatting'),hasDataValidation:!!localOne(doc,'dataValidation')});
  }
...
function patchHyperlinks(doc,sheet,rids){
  const root=doc.documentElement;for(const old of localAll(root,'hyperlinks'))old.remove();const links=[...sheet.cells].filter(([,cell])=>cell?.hyperlink?.target);if(!links.length)return;const node=create(doc,'hyperlinks');for(const[ref,cell]of links){const h=create(doc,'hyperlink');h.setAttribute('ref',ref);const link=cell.hyperlink;if(link.external){const rid=rids.get(ref);if(rid)h.setAttributeNS(REL,'r:id',rid)}else h.setAttribute('location',String(link.target));if(link.tooltip)h.setAttribute('tooltip',String(link.tooltip));node.appendChild(h)}const merge=localOne(root,'mergeCells'),pageMargins=localOne(root,'pageMargins'),drawing=localOne(root,'drawing');root.insertBefore(node,pageMargins||drawing||merge?.nextSibling||null)
}
async function syncHyperlinkRelationships(zip,sheet){
  const links=[...sheet.cells].filter(([,cell])=>cell?.hyperlink?.target&&cell.hyperlink.external),path=relsPathFor(sheet.path);let raw=await zip.file(path)?.async('text'),doc=raw?xml(raw,path):xml(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="${PKG_REL}"></Relationships>`,path),root=doc.documentElement;for(const rel of [...localAll(root,'Relationship')])if((rel.getAttribute('Type')||'').endsWith('/hyperlink'))rel.remove();const used=new Set(localAll(root,'Relationship').map(r=>r.getAttribute('Id')||'')),rids=new Map();let n=1;for(const[ref,cell]of links){while(used.has('rIdHyperlink'+n))n++;const rid='rIdHyperlink'+n++;used.add(rid);const rel=doc.createElementNS(PKG_REL,'Relationship');rel.setAttribute('Id',rid);rel.setAttribute('Type','http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink');rel.setAttribute('Target',String(cell.hyperlink.target));rel.setAttribute('TargetMode','External');root.appendChild(rel);rids.set(ref,rid)}if(root.children.length)zip.file(path,serializeXml(doc),{createFolders:true});else zip.remove(path);return rids
}
function patchAutoFilter(doc,sheet){if(String(sheet.originalAutoFilter||'')===String(sheet.autoFilter||''))return;const root=doc.documentElement;for(const old of localAll(root,'autoFilter'))old.remove();if(!sheet.autoFilter)return;const node=create(doc,'autoFilter');node.setAttribute('ref',String(sheet.autoFilter));const sheetData=localOne(root,'sheetData'),merge=localOne(root,'mergeCells'),hyperlinks=localOne(root,'hyperlinks'),pageMargins=localOne(root,'pageMargins');root.insertBefore(node,hyperlinks||pageMargins||merge?.nextSibling||sheetData?.nextSibling||null)}
function patchLegacyDrawing(doc,rid){const root=doc.documentElement;let node=localOne(root,'legacyDrawing');if(!rid){if(node)node.remove();return}if(!node){node=create(doc,'legacyDrawing');const extLst=localOne(root,'extLst');root.insertBefore(node,extLst||null)}node.setAttributeNS(REL,'r:id',rid)}
function patchSheetXml(raw,sheet,rids=new Map(),legacyDrawingRid=''){const doc=xml(raw);patchRowsAndCells(doc,sheet);patchColumns(doc,sheet);patchMerges(doc,sheet);patchAutoFilter(doc,sheet);patchHyperlinks(doc,sheet,rids);patchLegacyDrawing(doc,legacyDrawingRid);return'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'+serializeXml(doc).replace(/^<\?xml[^>]*>\s*/,'')}
function commentPartXml(comments){const authors=[...new Set(comments.map(([,c])=>c.comment.author||'InkDOS'))];return`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><comments xmlns="${NS}"><authors>${authors.map(a=>`<author>${escapeXml(a)}</author>`).join('')}</authors><commentList>${comments.map(([ref,c])=>`<comment ref="${ref}" authorId="${authors.indexOf(c.comment.author||'InkDOS')}"><text><t xml:space="preserve">${escapeXml(c.comment.text)}</t></text></comment>`).join('')}</commentList></comments>`}
function commentVmlXml(comments){return`<?xml version="1.0" encoding="UTF-8"?><xml xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"><o:shapelayout v:ext="edit"><o:idmap v:ext="edit" data="1"/></o:shapelayout><v:shapetype id="_x0000_t202" coordsize="21600,21600" o:spt="202" path="m,l,21600r21600,l21600,xe"><v:stroke joinstyle="miter"/><v:path gradientshapeok="t" o:connecttype="rect"/></v:shapetype>${comments.map(([ref],i)=>{const p=decodeRef(ref);return`<v:shape id="_x0000_s${1025+i}" type="#_x0000_t202" style="position:absolute;margin-left:80pt;margin-top:5pt;width:108pt;height:59.25pt;z-index:${i+1};visibility:hidden" fillcolor="#ffffe1" o:insetmode="auto"><v:fill color2="#ffffe1"/><v:shadow on="t" color="black" obscured="t"/><v:path o:connecttype="none"/><v:textbox style="mso-direction-alt:auto"><div style="text-align:left"/></v:textbox><x:ClientData ObjectType="Note"><x:MoveWithCells/><x:SizeWithCells/><x:Anchor>${p.c}, 15, ${p.r}, 2, ${p.c+2}, 31, ${p.r+3}, 1</x:Anchor><x:AutoFill>False</x:AutoFill><x:Row>${p.r}</x:Row><x:Column>${p.c}</x:Column></x:ClientData></v:shape>`}).join('')}</xml>`}
async function ensureCommentContentType(zip,commentPath,present){const path='[Content_Types].xml',raw=await zip.file(path)?.async('text');if(!raw)return;const doc=xml(raw,path),root=doc.documentElement,part='/'+commentPath;for(const n of [...localAll(root,'Override')])if(n.getAttribute('PartName')===part&&!present)n.remove();if(present&&!localAll(root,'Override').some(n=>n.getAttribute('PartName')===part)){const n=doc.createElementNS(root.namespaceURI,'Override');n.setAttribute('PartName',part);n.setAttribute('ContentType','application/vnd.openxmlformats-officedocument.spreadsheetml.comments+xml');root.appendChild(n)}zip.file(path,serializeXml(doc),{createFolders:false})}
async function syncCommentParts(zip,sheet){const comments=[...sheet.cells].filter(([,c])=>c?.comment?.text),relsPath=relsPathFor(sheet.path);let raw=await zip.file(relsPath)?.async('text'),doc=raw?xml(raw,relsPath):xml(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="${PKG_REL}"></Relationships>`,relsPath),root=doc.documentElement;let commentsRel=localAll(root,'Relationship').find(r=>(r.getAttribute('Type')||'').endsWith('/comments')),vmlRel=localAll(root,'Relationship').find(r=>(r.getAttribute('Type')||'').endsWith('/vmlDrawing'));const sheetNo=(sheet.path.match(/sheet(\d+)\.xml$/)||[])[1]||'1',commentPath=commentsRel?normalizePath(sheet.path,commentsRel.getAttribute('Target')||''):`xl/comments${sheetNo}.xml`,vmlPath=vmlRel?normalizePath(sheet.path,vmlRel.getAttribute('Target')||''):`xl/drawings/vmlDrawing${sheetNo}.vml`;if(!comments.length){if(commentsRel){commentsRel.remove();zip.remove(commentPath)}await ensureCommentContentType(zip,commentPath,false);if(vmlRel){vmlRel.remove();zip.remove(vmlPath)}if(root.children.length)zip.file(relsPath,serializeXml(doc),{createFolders:true});else zip.remove(relsPath);return''}const used=new Set(localAll(root,'Relationship').map(r=>r.getAttribute('Id')||''));const nextRid=prefix=>{let n=1;while(used.has(prefix+n))n++;const id=prefix+n;used.add(id);return id};if(!commentsRel){commentsRel=doc.createElementNS(PKG_REL,'Relationship');commentsRel.setAttribute('Id',nextRid('rIdComments'));commentsRel.setAttribute('Type','http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments');commentsRel.setAttribute('Target',`../comments${sheetNo}.xml`);root.appendChild(commentsRel)}if(!vmlRel){vmlRel=doc.createElementNS(PKG_REL,'Relationship');vmlRel.setAttribute('Id',nextRid('rIdVml'));vmlRel.setAttribute('Type','http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing');vmlRel.setAttribute('Target',`../drawings/vmlDrawing${sheetNo}.vml`);root.appendChild(vmlRel)}zip.file(commentPath,commentPartXml(comments),{createFolders:true});zip.file(vmlPath,commentVmlXml(comments),{createFolders:true});await ensureCommentContentType(zip,commentPath,true);zip.file(relsPath,serializeXml(doc),{createFolders:true});return vmlRel.getAttribute('Id')||''}
function sheetHasChanges(sheet){const refs=new Set([...sheet.originalCells.keys(),...sheet.cells.keys()]);for(const ref of refs)if(!sameCell(sheet.originalCells.get(ref)||null,sheet.cells.get(ref)||null))return true;return !sameObject(sheet.originalWidths,sheet.widths)||!sameObject(sheet.originalHeights,sheet.heights)||!sameObject(sheet.originalMerges,sheet.merges)||String(sheet.originalAutoFilter||'')!==String(sheet.autoFilter||'')}
function styleSignature(style){return JSON.stringify(style||{})}
function prepareGeneratedStyles(book){
  const styles=(book.legacyStyles&&book.legacyStyles.length?book.legacyStyles.map(x=>JSON.parse(JSON.stringify(x||{}))):[{}]),index=new Map(styles.map((st,i)=>[styleSignature(st),i]));
  for(const sheet of book.sheets||[])for(const cell of sheet.cells.values())if(cell&&cell._styleDirty){const sig=styleSignature(cell.style);let id=index.get(sig);if(id===undefined){id=styles.length;styles.push(JSON.parse(JSON.stringify(cell.style||{})));index.set(sig,id)}cell.styleId=id;delete cell._styleDirty}
  book.legacyStyles=styles;
}
function appendStyle(doc,style){
  const root=doc.documentElement,fonts=localOne(root,'fonts'),fills=localOne(root,'fills'),borders=localOne(root,'borders'),cellXfs=localOne(root,'cellXfs');if(!fonts||!fills||!borders||!cellXfs)return 0;
  const st=style||{},f=st.font||{},font=create(doc,'font');const sz=create(doc,'sz');sz.setAttribute('val',String(Number(f.size)||11));font.appendChild(sz);const name=create(doc,'name');name.setAttribute('val',String(f.name||'Calibri'));font.appendChild(name);if(f.bold)font.appendChild(create(doc,'b'));if(f.italic)font.appendChild(create(doc,'i'));if(f.underline)font.appendChild(create(doc,'u'));if(f.strike)font.appendChild(create(doc,'strike'));if(f.color){const c=create(doc,'color');c.setAttribute('rgb',String(f.color).replace('#','').padStart(8,'FF'));font.appendChild(c)}fonts.appendChild(font);fonts.setAttribute('count',String(children(fonts,'font').length));const fontId=children(fonts,'font').length-1;
  const fill=create(doc,'fill'),pattern=create(doc,'patternFill');pattern.setAttribute('patternType',st.fill?'solid':'none');if(st.fill){const fg=create(doc,'fgColor');fg.setAttribute('rgb',String(st.fill).replace('#','').padStart(8,'FF'));pattern.appendChild(fg);const bg=create(doc,'bgColor');bg.setAttribute('indexed','64');pattern.appendChild(bg)}fill.appendChild(pattern);fills.appendChild(fill);fills.setAttribute('count',String(children(fills,'fill').length));const fillId=children(fills,'fill').length-1;
  const border=create(doc,'border');for(const sideName of['left','right','top','bottom']){const side=create(doc,sideName),src=st.border?.[sideName];if(src?.style)side.setAttribute('style',src.style);if(src?.color){const c=create(doc,'color');c.setAttribute('rgb',String(src.color).replace('#','').padStart(8,'FF'));side.appendChild(c)}border.appendChild(side)}border.appendChild(create(doc,'diagonal'));borders.appendChild(border);borders.setAttribute('count',String(children(borders,'border').length));const borderId=children(borders,'border').length-1;
  const xf=create(doc,'xf');xf.setAttribute('numFmtId',String(Number(st.numFmtId)||0));xf.setAttribute('fontId',String(fontId));xf.setAttribute('fillId',String(fillId));xf.setAttribute('borderId',String(borderId));xf.setAttribute('xfId','0');xf.setAttribute('applyFont','1');xf.setAttribute('applyFill','1');xf.setAttribute('applyBorder','1');const attrs={horizontal:st.align&&st.align!=='general'?st.align:'',vertical:st.vertical||'',wrapText:st.wrap?'1':'',textRotation:st.rotation?String(st.rotation<0?180+st.rotation:st.rotation):''};if(Object.values(attrs).some(Boolean)){const al=create(doc,'alignment');for(const[k,v]of Object.entries(attrs))if(v)al.setAttribute(k,v);xf.appendChild(al);xf.setAttribute('applyAlignment','1')}cellXfs.appendChild(xf);cellXfs.setAttribute('count',String(children(cellXfs,'xf').length));return children(cellXfs,'xf').length-1;
}
async function ensureImportedStyleIds(zip,book){
  const dirty=[];for(const sheet of book.sheets||[])for(const cell of sheet.cells.values())if(cell&&cell._styleDirty)dirty.push(cell);if(!dirty.length)return false;const raw=await zip.file('xl/styles.xml')?.async('text');if(!raw)return false;const doc=xml(raw,'xl/styles.xml'),seen=new Map();for(const cell of dirty){const sig=styleSignature(cell.style);let id=seen.get(sig);if(id===undefined){id=appendStyle(doc,cell.style);seen.set(sig,id)}cell.styleId=id;delete cell._styleDirty}const stylesXml='<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'+serializeXml(doc).replace(/^<\?xml[^>]*>\s*/,'');zip.file('xl/styles.xml',stylesXml,{createFolders:false});
  // Cells now reference the appended xf ids; keep the session package in step so later saves (after a cancelled, failed or successful delivery) still contain them.
  if(book.zip&&book.zip!==zip)book.zip.file('xl/styles.xml',stylesXml,{createFolders:false});return true
}
function serializeSheet(sheet,drawingRid=''){
  ...
  let hyperlinkRid=0;const hyperlinkEntries=[...sheet.cells].filter(([,cell])=>cell?.hyperlink?.target),hyperlinkXml=hyperlinkEntries.length?`<hyperlinks>${hyperlinkEntries.map(([ref,cell])=>cell.hyperlink.external?`<hyperlink ref="${ref}" r:id="rIdHyperlink${++hyperlinkRid}"${cell.hyperlink.tooltip?` tooltip="${escapeXml(cell.hyperlink.tooltip)}"`:''}/>`:`<hyperlink ref="${ref}" location="${escapeXml(cell.hyperlink.target)}"${cell.hyperlink.tooltip?` tooltip="${escapeXml(cell.hyperlink.tooltip)}"`:''}/>`).join('')}</hyperlinks>`:'';
  ...
  return`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="${NS}" xmlns:r="${REL}"><dimension ref="${usedRange(sheet)}"/>${pane}${cols}<sheetFormatPr defaultRowHeight="${pxHeight(sheet.defaultRowHeight||20).toFixed(2)}"/><sheetData>${rowXml}</sheetData>${mergeXml}${hyperlinkXml}${margins}${setup}${hf}${drawing}</worksheet>`;
}
...
function drawingXmlForSheet(sheet,index,mediaStart){
  const objects=[],rels=[],media=[],charts=[];let id=1,mediaNo=mediaStart;
  for(const d of sheet.drawings||[]){const from=d.from||{c:0,r:0,x:0,y:0},to=d.to||{c:(from.c||0)+1,r:(from.r||0)+1,x:0,y:0};if(d.kind==='image'&&d.bytes){const ext=(d.extension||'png').replace(/^jpeg$/,'jpg'),rid=`rId${rels.length+1}`,name=`image${mediaNo}.${ext}`;media.push({path:`xl/media/${name}`,bytes:d.bytes,extension:ext,mime:d.mime||`image/${ext}`});rels.push({id:rid,target:`../media/${name}`});objects.push(`<xdr:twoCellAnchor editAs="oneCell">${markerXml('from',from)}${markerXml('to',to)}<xdr:pic><xdr:nvPicPr><xdr:cNvPr id="${id}" name="${escapeXml(d.name||`Picture ${id}`)}"/><xdr:cNvPicPr/></xdr:nvPicPr><xdr:blipFill><a:blip r:embed="${rid}"/><a:stretch><a:fillRect/></a:stretch></xdr:blipFill><xdr:spPr><a:xfrm/><a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:noFill/></xdr:spPr></xdr:pic><xdr:clientData/></xdr:twoCellAnchor>`);mediaNo++;id++;continue}if(d.kind==='chart'&&Array.isArray(d.series)&&d.series.length){const rid=`rId${rels.length+1}`,chartNo=index*100+charts.length+1,chartPath=`xl/charts/chart${chartNo}.xml`;rels.push({id:rid,target:`../charts/chart${chartNo}.xml`,type:'chart'});const chartTag=['lineChart','pieChart'].includes(d.chartType)?d.chartType:'barChart',seriesXml=d.series.map((ser,si)=>{const cat=escapeXml(ser.categoryFormula||''),val=escapeXml(ser.valueFormula||''),name=escapeXml(ser.nameFormula||'');return`<c:ser><c:idx val="${si}"/><c:order val="${si}"/>${name?`<c:tx><c:strRef><c:f>${name}</c:f></c:strRef></c:tx>`:''}<c:cat><c:strRef><c:f>${cat}</c:f></c:strRef></c:cat><c:val><c:numRef><c:f>${val}</c:f></c:numRef></c:val></c:ser>`}).join(''),chartBody=chartTag==='barChart'?`<c:barChart><c:barDir val="col"/><c:grouping val="clustered"/>${seriesXml}<c:axId val="48650112"/><c:axId val="48672768"/></c:barChart><c:catAx><c:axId val="48650112"/><c:scaling><c:orientation val="minMax"/></c:scaling><c:axPos val="b"/><c:crossAx val="48672768"/></c:catAx><c:valAx><c:axId val="48672768"/><c:scaling><c:orientation val="minMax"/></c:scaling><c:axPos val="l"/><c:crossAx val="48650112"/></c:valAx>`:`<c:${chartTag}>${seriesXml}</c:${chartTag}>`;charts.push({path:chartPath,xml:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><c:chartSpace xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><c:chart><c:autoTitleDeleted val="0"/><c:plotArea><c:layout/>${chartBody}</c:plotArea><c:plotVisOnly val="1"/></c:chart></c:chartSpace>`});objects.push(`<xdr:twoCellAnchor editAs="oneCell">${markerXml('from',from)}${markerXml('to',to)}<xdr:graphicFrame macro=""><xdr:nvGraphicFramePr><xdr:cNvPr id="${id}" name="${escapeXml(d.name||`Chart ${id}`)}"/><xdr:cNvGraphicFramePr/></xdr:nvGraphicFramePr><xdr:xfrm/><a:graphic><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/chart"><c:chart xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart" xmlns:r="${REL}" r:id="${rid}"/></a:graphicData></a:graphic><xdr:clientData/></xdr:graphicFrame></xdr:twoCellAnchor>`);id++;continue}if(d.kind==='shape'&&String(d.text||'').trim()){const paras=String(d.text).split(/\n/).map(line=>`<a:p><a:r><a:rPr lang="en-US" sz="${Math.round(Number(d.fontSize||10)*100)}"${d.bold?' b="1"':''}/><a:t>${escapeXml(line)}</a:t></a:r><a:endParaRPr lang="en-US"/></a:p>`).join('');objects.push(`<xdr:twoCellAnchor editAs="oneCell">${markerXml('from',from)}${markerXml('to',to)}<xdr:sp><xdr:nvSpPr><xdr:cNvPr id="${id}" name="${escapeXml(d.name||`Text Box ${id}`)}"/><xdr:cNvSpPr txBox="1"/><xdr:nvPr/></xdr:nvSpPr><xdr:spPr><a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:noFill/><a:ln><a:noFill/></a:ln></xdr:spPr><xdr:txBody><a:bodyPr wrap="square"/><a:lstStyle/>${paras}</xdr:txBody></xdr:sp><xdr:clientData/></xdr:twoCellAnchor>`);id++}}
  if(!objects.length)return null;const xml=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="${REL}">${objects.join('')}</xdr:wsDr>`,relsXml=rels.length?`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="${PKG_REL}">${rels.map(x=>`<Relationship Id="${x.id}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/${x.type||'image'}" Target="${x.target}"/>`).join('')}</Relationships>`:'';return{xml,relsXml,media,charts,nextMedia:mediaNo,path:`xl/drawings/drawing${index+1}.xml`,relsPath:`xl/drawings/_rels/drawing${index+1}.xml.rels`};
}
async function buildNewPackage(book){
  ...
  for(const x of sheetEntries){const d=drawings[x.i],drawingRid=d?'rIdDrawing1':'';x.s.path=x.path;zip.file(x.path,serializeSheet(x.s,drawingRid));const rels=[];if(d){zip.file(d.path,d.xml);if(d.relsXml)zip.file(d.relsPath,d.relsXml);for(const m of d.media)zip.file(m.path,m.bytes);for(const c of d.charts||[])zip.file(c.path,c.xml);rels.push(`<Relationship Id="rIdDrawing1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing" Target="../drawings/drawing${x.i+1}.xml"/>`)}let hn=1;for(const[,cell]of x.s.cells)if(cell?.hyperlink?.target&&cell.hyperlink.external){rels.push(`<Relationship Id="rIdHyperlink${hn++}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="${escapeXml(cell.hyperlink.target)}" TargetMode="External"/>`)}const comments=[...x.s.cells].filter(([,cell])=>cell?.comment?.text);if(comments.length){const authors=[...new Set(comments.map(([,c])=>c.comment.author||'InkDOS'))],commentPath=`xl/comments${x.i+1}.xml`;zip.file(commentPath,`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><comments xmlns="${NS}"><authors>${authors.map(a=>`<author>${escapeXml(a)}</author>`).join('')}</authors><commentList>${comments.map(([ref,c])=>`<comment ref="${ref}" authorId="${authors.indexOf(c.comment.author||'InkDOS')}"><text><t xml:space="preserve">${escapeXml(c.comment.text)}</t></text></comment>`).join('')}</commentList></comments>`);rels.push(`<Relationship Id="rIdComments1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments" Target="../comments${x.i+1}.xml"/>`)}if(rels.length)zip.file(`xl/worksheets/_rels/sheet${x.i+1}.xml.rels`,`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="${PKG_REL}">${rels.join('')}</Relationships>`)}
...
async function saveCopy(book){
  if(!book.loaded)throw new Error('Create or open a workbook before saving a copy');if(!book.zip){prepareGeneratedStyles(book);return buildNewPackage(book);}
  const source=await book.zip.generateAsync({type:'uint8array'}),zip=await JSZip.loadAsync(source,{checkCRC32:true});let changedAny=await ensureImportedStyleIds(zip,book);changedAny=(await global.InkDOS2SpreadsheetWorksheetPackage.syncSheets(zip,book,{serializeSheet}))||changedAny;
  for(const s of book.sheets){if(!sheetHasChanges(s))continue;const raw=await zip.file(s.path)?.async('text');if(!raw)continue;const hyperlinkRids=await syncHyperlinkRelationships(zip,s),legacyDrawingRid=await syncCommentParts(zip,s);zip.file(s.path,patchSheetXml(raw,s,hyperlinkRids,legacyDrawingRid),{createFolders:false});changedAny=true}
