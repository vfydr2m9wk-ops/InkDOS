...
function create({session,setLoading,onStatus,onError,onSessionChanged}={}){let saveId=0;
 ...
 async function share(){if(!session.book?.loaded)return false;const id=++saveId,fileName=NS.FileDelivery.safeName(session.fileName);try{setLoading?.(true,'Preparing XLSX to share…');NS.FormulaEvaluator.recalculate(session.book);const blob=await root.LocalXLSX.saveCopy(session.book);if(id!==saveId)return false;const result=await NS.FileDelivery.share(blob,fileName);onStatus?.('XLSX sent to Share Sheet');return result}catch(error){if(error?.code==='cancelled')onStatus?.('Share cancelled');else onError?.(error);return false}finally{setLoading?.(false)}}
