...
t1=NS.TxtT1Essentials.create({elements:E,state,editor,files,controls,commands});controls.bind({editor,files,state,commands},frameMenu);t1.install();t2=NS.TxtT2XmlTools.create({elements:E,state,editor,files});t2.install();NS.TxtAppDebug=Object.freeze({state,commands,...editor.debug(),...files.debug(),txtT1:t1.debug(),txtT2:t2.debug()});files.initialize();
})(globalThis);
