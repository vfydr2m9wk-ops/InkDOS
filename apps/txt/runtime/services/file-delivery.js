...
function capabilities(file){return Object.freeze({fileSystem:typeof g.showSaveFilePicker==='function',share:canShareFile(file),download:!!(g.document&&g.URL&&URL.createObjectURL)})}
...
function chooseRoute(file){const c=capabilities(file);const local=(g.location&&g.location.protocol==='file:');if(local&&c.share)return 'web-share';if(c.fileSystem)return 'file-system-access';if(c.share)return 'web-share';return 'download'}
...
function deliver(blob,fileName){const file=toFile(blob,fileName);const route=chooseRoute(file);/* The native route is invoked before any await so transient user activation is still available. */
...
if(route==='file-system-access')return viaPicker(blob,fileName).catch(e=>{if(e.code==='cancelled')throw e;if(canShareFile(file))return viaShare(blob,fileName,file).catch(()=>viaDownload(blob,fileName).catch(()=>{throw e}));return viaDownload(blob,fileName).catch(()=>{throw e})});
