...
function purgePackages(keep){for(const item of LANGUAGES){const code=item.code;if(code===DEFAULT_LANGUAGE||code===keep)continue;try{delete packages[code]}catch(_){}if(doc)doc.getElementById('inkdosLocalePackage-'+code)?.remove()}if(keep===DEFAULT_LANGUAGE){for(const item of LANGUAGES){if(item.code===DEFAULT_LANGUAGE)continue;try{delete packages[item.code]}catch(_){}if(doc)doc.getElementById('inkdosLocalePackage-'+item.code)?.remove()}}}
