...
function loadPackage(value){const code=normalizeLanguage(value);if(code===DEFAULT_LANGUAGE)return Promise.resolve(null);const existing=packageFor(code);if(existing)return Promise.resolve(existing);if(!doc)return Promise.reject(new Error('Locale package requires a document environment.'));return new Promise((resolve,reject)=>{const marker='inkdosLocalePackage-'+code;const prior=doc.getElementById(marker);if(prior){prior.addEventListener('load',()=>resolve(packageFor(code)||Object.freeze({})),{once:true});prior.addEventListener('error',()=>reject(new Error('Unable to load locale '+code)),{once:true});return}const script=doc.createElement('script');script.id=marker;script.src=base+'locales/'+packagePath(code);script.async=true;script.addEventListener('load',()=>resolve(packageFor(code)||Object.freeze({})),{once:true});script.addEventListener('error',()=>reject(new Error('Unable to load locale '+code)),{once:true});(doc.head||doc.documentElement).appendChild(script)})}
function persist(code){if(!storageKey)return;try{root.localStorage?.setItem(storageKey,code)}catch(_){}}
async function setLanguage(value,options){const code=normalizeLanguage(value);if(options&&typeof options.storageKey==='string')storageKey=options.storageKey;language=code;persist(code);if(code===DEFAULT_LANGUAGE){dictionary=Object.freeze({});apply();return language}try{dictionary=await loadPackage(code)}catch(_){language=DEFAULT_LANGUAGE;dictionary=Object.freeze({});persist(language)}apply();return language}
...
observe();
root.InkDOSLocalization=Object.freeze({DEFAULT_LANGUAGE,ALLOWED_ATTRIBUTES,languages:LANGUAGES,normalizeLanguage,packagePath,translateValue,loadPackage,setLanguage,readLanguage,install,apply,get currentLanguage(){return language},get storageKey(){return storageKey}});
})(globalThis);
