...
const STYLE_PROPS=new Set(['font-family','font-size','font-weight','font-style','text-decoration','color','background-color','text-align','line-height','margin-left','margin-right','text-indent','width','height','vertical-align','white-space']);
...
function safeStyle(value){const out=[];for(const declaration of String(value||'').split(';')){const pos=declaration.indexOf(':');if(pos<1)continue;const prop=declaration.slice(0,pos).trim().toLowerCase(),val=declaration.slice(pos+1).trim();if(!STYLE_PROPS.has(prop)||!val||/[<>{}]|url\s*\(|expression\s*\(|@import|javascript:/i.test(val))continue;if(prop==='font-family'&&!/^[\w\s,'".-]+$/.test(val))continue;if(['color','background-color'].includes(prop)&&!/^(?:#[0-9a-f]{3,8}|rgba?\([\d\s.,%]+\)|[a-z]{1,20})$/i.test(val))continue;if(['width','height','font-size','margin-left','margin-right','text-indent'].includes(prop)&&!/^\d+(?:\.\d+)?(?:px|pt|em|rem|%)$/.test(val))continue;out.push(prop+':'+val)}return out.join(';')}
