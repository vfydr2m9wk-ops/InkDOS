...
class RefParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        data = dict(attrs)
        is_home = tag == "a" and data.get("aria-label") == "Home"
        for key in ("src", "href"):
            value = data.get(key)
            if value:
                self.refs.append((tag, key, value, is_home))
...
def validate_isolated_copy(app, errors):
    ...
    with tempfile.TemporaryDirectory(prefix=f"inkdos-{app}-") as td:
        ...
        for tag, key, value, is_home in parser.refs:
            ...
            if is_home:
                if path != "../../index.html":
                    errors.append(f"{app}: unexpected Home target {value}")
                continue
    ...
    for path in root.rglob("*"):
        ...
        for pattern in patterns:
            if pattern.search(text):
                errors.append(f"{app}: cross-app source reference in {rel}: {pattern.pattern}")
        ...
        for marker in forbidden_roots:
            if re.search(rf"(?:\.\./)+{re.escape(marker)}", text):
...
def validate_optional_home(app, errors):
    ...
    if not frame_path.is_file():
        errors.append(f"{app}: app-local frame module missing: {HOME_FRAME[app]}")
        return
    ...
    for marker in ("configureOptionalHome", "URLSearchParams", "get('suite')==='1'", "removeAttribute('href')"):
        if marker not in frame:
            errors.append(f"{app}: optional Home gate missing marker {marker}")
...
def validate_home_launcher(errors):
    ...
    for app in ACTIVE:
        ...
        if expected not in home:
            errors.append(f"Home: suite opt-in route missing for {app}")


def validate_service_worker_shell(errors):
    service_worker = (ROOT / "service-worker.js").read_text(encoding="utf-8")
    refs = re.findall(r"[\"'](\./[^\"']+)[\"']", service_worker)
    for ref in refs:
...
def validate_epub_layout(errors):
    ...
    for rel in EPUB_LAYOUT:
        if not (root / rel).is_file():
            errors.append(f"epub: expected modular file missing: {rel}")
    for rel in EPUB_RETIRED_ROOT_FILES:
        if (root / rel).exists():
            errors.append(f"epub: retired flat root file still present: {rel}")
    index = (root / "index.html").read_text(encoding="utf-8")
    for rel in (
        "io/package-reader.js", "engine/content-projector.js", "engine/book-model.js",
        "state/annotation-store.js", "engine/annotations.js", "io/epub-writer.js",
        "view/renderer.js", "ui/start-state.css",
    ):
        if rel not in index:
            errors.append(f"epub: modular index reference missing: {rel}")
