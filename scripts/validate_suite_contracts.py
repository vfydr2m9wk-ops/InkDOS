...
class SuiteIntegration(unittest.TestCase):
    ...
    def test_standard_start_cards(self):
        ...
        for app in two_action:
            ...
            self.assertIn("start-card", text, app)
            self.assertIn('id="startNew"', text, app)
            self.assertIn('id="startOpen"', text, app)
        epub = (ROOT / "apps/epub/index.html").read_text(encoding="utf-8")
        self.assertIn("start-card", epub)
        self.assertIn('id="openStartBtn"', epub)
        self.assertNotIn('id="startNew"', epub)
    ...
    def test_pdf_active(self):
        text = (ROOT / "index.html").read_text(encoding="utf-8")
        self.assertIn("PDF Workspace", text)
        self.assertNotIn("Coming soon", text)
        self.assertTrue((ROOT / "apps/pdf/index.html").is_file())
    ...
    def test_return_bridge(self):
        for app in ACTIVE:
            ...
            self.assertIn('aria-label="Home"', text)
    ...
    def test_share_action_contract(self):
        ...
        for app, (rel, marker) in {**direct, **runtime}.items():
            ...
            self.assertIn(marker, text, app)
            self.assertIn("Share", text, app)
    ...
    def test_empty_state_file_action_contract(self):
        ...
        self.assertIn("legacy-ppt-to-editable-pptx", presentation_save)
        self.assertIn("n.replace(/\\.ppt$/i,'.pptx')", presentation_save)
        txt_editor = (ROOT / "apps/txt/editor/editor-controller.js").read_text(encoding="utf-8")
        txt_controls = (ROOT / "apps/txt/ui/txt-controls.js").read_text(encoding="utf-8")
        self.assertIn("document.dispatchEvent(new CustomEvent('inkdos:txt-command-state'", txt_editor)
        self.assertNotIn("E.saveBtn", txt_editor)
        self.assertNotIn("E.shareBtn", txt_editor)
        self.assertIn("document.addEventListener('inkdos:txt-command-state',syncCommandState)", txt_controls)
        self.assertIn("if(E.saveBtn)E.saveBtn.disabled=!detail.loaded", txt_controls)
        self.assertIn("if(E.shareBtn)E.shareBtn.disabled=!detail.loaded", txt_controls)
        ...
        self.assertIn("function initialize(){initializeEmptyState()}", txt_files)
        self.assertNotIn("function initialize(){initializeEmptyState();restoreRecovery()}", txt_files)
        self.assertIn("restoreRecovery", txt_files)
        self.assertIn("if(!state.loaded||state.largeFile)return false", txt_files)
    ...
    def test_mobile_home_layout(self):
        ...
        self.assertIn(".workspace-grid{grid-template-columns:1fr", css)

    def test_no_cross_suite_runtime_roots(self):
        for rel in ("modules", "core"):
