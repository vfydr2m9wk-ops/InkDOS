...
InkDOS uses one stable workflow:
...
stable workflow manually in a normal repository commit. This is a one-time
bootstrap operation.
...
1. Place exactly one `InkDOS-update-v*.zip` in the repository root.
2. Run **InkDOS integrity and update** from the Actions tab.
...
If validation fails, the validated candidate is discarded and the repository
remains unchanged. If the final push fails, the remote repository and its root
