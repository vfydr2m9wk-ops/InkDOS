...
| PDF.js | 3.11.174 | Local PDF parsing, rendering, text, forms and supported document saving; classic build supports direct local-file startup | `shared/vendor/pdfjs/pdf.min.js` | Apache-2.0 | Recorded in `RUNTIME_CHECKSUMS.sha256` |
...
All three libraries work offline. PDF.js runs with its bundled local worker and makes no CDN request.
